        ...
        <div id="lov{{$index}}" class="flex-col lov-container" ng-repeat="lov in lovsVm.lovs">
            <div id="lovMask{{$index}}" class="lov-container-mask centered-container">
                <div class="centered-child">
                    <h5>Please confirm to delete this LOV</h5>

                    <div>
                        <button class='btn btn-xs btn-default' ng-click='lovsVm.hideMask($index)'>No</button>
                        <button class='btn btn-xs btn-danger' ng-click='lovsVm.deleteLov(lov)'>Yes</button>
                    </div>
                </div>
            </div>
            <div id="lovValueMask{{$index}}" class="lov-container-mask centered-container">
                <div class="centered-child">
                    <h5>Please confirm to delete this value</h5>

                    <div>
                        <button class='btn btn-xs btn-default' ng-click='lovsVm.hideMask($index)'>No</button>
                        <button class='btn btn-xs btn-danger' ng-click='lovsVm.deleteLovValue(lov)'>Yes
                        </button>
                    </div>
                </div>
            </div>
            <div class="lov-header">
                <span ng-if="!lov.editTitle" ng-dblclick="lov.editTitle = true;
                $('#titleInput{{$index}}').setSelectionRange(0, this.value.length)">{{lov.newName}}</span>
                ...
                <div class="buttons" ng-if="!lov.editTitle">
                    <i ng-if="lov.id != null && hasPermission('permission.settings.edit')" title="Add New Value"
                       class="fa fa-plus mr10"
                       ng-click="lovsVm.addLovValue(lov, $index)"></i>
                    <i title="Delete this Lov" class="fa fa-times" ng-if="hasPermission('permission.settings.edit')"
            ...
            <div class="lov-body" id="lov-body{{$index}}">
                <div class="lov-value" ng-repeat="value in lov.valueObjects" ng-class="{'edit-mode': value.editMode}">
                    <div ng-if="value.editMode" style="display: flex">
                        <span class="name" style="flex-grow: 1">
                            <input type="text"
                                   ng-model="value.newString"
                                   ng-enter="value.editMode = false;value.string = value.newString;lovsVm.applyChanges(lov)"
                                   onfocus="this.setSelectionRange(0, this.value.length)">
                        </span>
                        <span class="buttons">
                            <i title="Save Changes" class="fa fa-check mr5"
                               ng-click="value.editMode = false;value.newMode = false;value.string = value.newString;lovsVm.applyChanges(lov)">
                            </i>
                            <i title="Cancel Changes"
                               ng-click="value.editMode = false;value.newString = value.string; lovsVm.cancelChanges(lov, value)"
                               class="fa fa-times"></i>
                        </span>
                    </div>
                    ...
                    <div ng-if="!value.editMode">
                        ...
                        <span class="buttons">
                            ...
                            <i title="Delete Value" ng-if="hasPermission('permission.settings.delete')"
                               ng-click="lovsVm.toDeleteValue = value;lovsVm.promptDeleteLovValue($parent.$parent.$index)"
                               class="fa fa-times"></i>
                        </span>
                    </div>
                </div>
