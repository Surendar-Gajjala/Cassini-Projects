define(
    ...
    function (module) {
        module.controller('LotUpnHistoryController', LotUpnHistoryController);

        function LotUpnHistoryController($scope, $stateParams, $rootScope, $timeout, $interval, $state, $cookies, InwardService, ItemTypeService, AttributeAttachmentService) {

            var vm = this;

            vm.barcodeBust = (new Date()).getTime();
            vm.mode = $scope.data.lotUpnHistoryMode;
            vm.itemInstance = $scope.data.statusItemInstance;

            function loadUpnHistory() {
                if (vm.mode == "INWARD") {
                    InwardService.getLotUpnDetails(vm.itemInstance.id).then(
                        function (data) {
                            vm.upnDetails = data;
                            loadAttributes();
                        }
                    )
                } else if (vm.mode == "ISSUE") {
                    InwardService.getIssuedLotUpnDetails(vm.itemInstance.id).then(
                        function (data) {
                            vm.upnDetails = data;
                            loadAttributes();
                        }
            ...
            vm.showInfo = false;
            vm.showIssues = false;
            vm.showLotIssues = showLotIssues;
