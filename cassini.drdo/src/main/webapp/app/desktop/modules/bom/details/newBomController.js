define(
    ...
    function (module) {
        module.controller('NewBomController', NewBomController);

        function NewBomController($scope, $rootScope, $q, $timeout, $state, $stateParams, $uibModal, $cookies, $window, $translate,
            ...
            function performSearch() {
                ...
                if (vm.filters.searchQuery != "") {
                    vm.filters.item = $rootScope.selectedSystemDetails.id;
                    ...
                    ItemService.searchItems(pageable, vm.filters).then(
                        function (data) {
                            ...
                            ItemService.getRevisionReferences(vm.searchResults.content, 'latestRevision');
                        }
                    )
                }
            }
            ...
            function checkIfAlreadyAdded(result) {
                var found = false;
                angular.forEach($rootScope.addedSearchItems, function (item) {
            ...
            function addItemToBom(item) {
                $rootScope.addSearchItemToBom(item);
                $rootScope.addedSearchItems.push(item);
                var index = vm.searchResults.content.indexOf(item);
                vm.searchResults.content.splice(index, 1);
            }

            function addNew() {
                $rootScope.newBomItemButton = true;
            ...
            function onSelectType(itemType) {
                ...
                if (itemType != null && itemType != undefined) {
                    ...
                    vm.newBomItem.newItem.itemName = itemType.name;
                    vm.newBomItem.newItem.itemCode = itemType.typeCode;
                    vm.newBomItem.newItem.partSpec = null;
                    vm.newBomItem.newItem.drawingNumber = null;
                    vm.newBomItem.newItem.material = null;
                    vm.selectedTypeCode = itemType.typeCode;
                    $("#typeTree").hide();
            ...
            function addItem() {
                ...
                if ($rootScope.newBomItemButton === true) {
                    if (validateItem() && validateCustomAttributes() && validateTypeAttributes()) {
                        $rootScope.showBusyIndicator($("#rightSidePanel"));
                        vm.newBomItem.parent = selectedItem.id;
                        vm.newBomItem.bomItemType = 'PART';
                        if (selectedItem.bomItemType == 'SECTION' || selectedItem.bomItemType == "COMMONPART") {
                            vm.newBomItem.hierarchicalCode = selectedItem.hierarchicalCode + "" + selectedItem.typeRef.code + "" + "000" + "" + vm.newBomItem.newItem.itemCode;
                        } else if (selectedItem.bomItemType == 'SUBSYSTEM') {
                            vm.newBomItem.hierarchicalCode = selectedItem.hierarchicalCode + "" + selectedItem.typeRef.code + "" + "00" + "" + vm.newBomItem.newItem.itemCode;
                        } else if (selectedItem.bomItemType == 'UNIT') {
                            vm.newBomItem.hierarchicalCode = selectedItem.hierarchicalCode + "" + selectedItem.typeRef.code + "" + vm.newBomItem.newItem.itemCode;
                        }

                        if (vm.newBomItem.newItem.itemType.hasSpec) {
                            vm.newBomItem.hierarchicalCode = vm.newBomItem.hierarchicalCode + "" + vm.newBomItem.newItem.partSpec.specName;
                        }

                        if (vm.typeParentNode.name != "Part") {
                            vm.newBomItem.quantity = null;
                            vm.newBomItem.workCenter = null;
                            vm.newBomItem.fractionalQuantity = null;
                        }
                        vm.newBomItem.newItem.itemCode = vm.selectedTypeCode;
                                        ...
                                        saveAttributes().then(
                                            function (attributes) {
                                                ...
                                                vm.newBomItem = {
                                                    ...
                                                    newItem: {
                                                        id: null,
                                                        itemType: data.item.itemMaster.itemType,
                                                        itemNumber: null,
                                                        itemName: data.item.itemMaster.itemType.name,
                                                        description: null,
                                                        units: "Each",
                                                        drawingNumber: null,
                                                        itemCode: data.item.itemMaster.itemType.typeCode,
                                                        latestRevision: null,
                                                        partSpec: null,
                                                        material: null
                                                    },
                ...
                } else if ($rootScope.newBomItemButton === false) {
                    $scope.callback();
            ...
            function validateItem() {
                ...
                if (vm.newBomItem.newItem.itemType == null || vm.newBomItem.newItem.itemType == "") {
                ...
                } else if (vm.newBomItem.newItem.itemType.hasSpec && (vm.newBomItem.newItem.partSpec == null || vm.newBomItem.newItem.partSpec == "" || vm.newBomItem.newItem.partSpec == undefined)) {
                    valid = false;
                    ...
                    valid = false;
                    mess = "Please enter Quantity";
                } else if (vm.typeParentNode.name == "Part" && (vm.newBomItem.workCenter == null || vm.newBomItem.workCenter == "" || vm.newBomItem.workCenter == undefined)) {
                    valid = false;
                    mess = "Please select work center";
                }

                $timeout(function () {
                    if (!valid) {
                        $rootScope.showWarningMessage(mess);
                    }
                }, 300)
                return valid;
            }
            ...
            function loadItemTypes() {
                ItemTypeService.getAllItemTypesWithoutSystemType().then(
                    function (data) {
                        vm.itemTypes = data;
                    }
                )
            ...
            (function () {
                if ($application.homeLoaded == true) {
                    ...
                    loadItemTypes();
                    button.text = "Close";
                    $scope.$on('app.bomItem.new', addItem);
