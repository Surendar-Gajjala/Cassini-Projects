...
setGridXPosForDualYAxis(yTitleCoords, yaxisLabelCoords)
{
    ...
    w.config.yaxis.map((yaxe, index) = > {
        if (
            w.globals.ignoreYAxisIndexes.indexOf(index) === -1 && !w.config.yaxis[index].floating &&
...
titleSubtitleOffset()
{
    ...
    if (
        ...
        w.config.legend.position === 'bottom' && !w.config.legend.floating &&
        (w.config.series.length > 1 || !w.globals.axisCharts ||
...
getxAxisLabelsCoords()
{
    ...
    if (w.globals.timelineLabels.length > 0) {
        ...
        let lgWidthForSideLegends =
            ...
            w.config.legend.position === 'right' && !w.config.legend.floating
                ? this.lgRect.width
                : 0
        ...
        if (
            ...
            `rotate(${w.config.xaxis.labels.rotate} 0
                0
            )`,
                false
            )
...
getLegendsRect()
{
    ...
    if (
        elLegendWrap !== null && !w.config.legend.floating &&
        w.config.legend.show
    ) {
        this.lgRect = {
            x: lgRect.x,
