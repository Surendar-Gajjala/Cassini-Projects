...
.
    ...
    let
    tickAmount = w.globals.yAxisScale[realIndex].result.length - 1

    // labelsDivider is simply svg width/number of ticks
    let
    labelsDivider = w.globals.gridWidth / tickAmount + 0.1
    ...
    let
    lbFormatter = w.globals.xLabelFormatter

    let
    labels = w.globals.yAxisScale[realIndex].result.slice()

    let
    timelineLabels = w.globals.invertedTimelineLabels
...
>
        ...
        this
.
    xaxisLabels = timelineLabels.slice()
...
yAxisTitleRotate(realIndex, yAxisOpposite)
{
    ...
    let yAxisTitle = w.globals.dom.baseEl.querySelector(
      `.
    apexcharts - yaxis[rel = '${realIndex}'].apexcharts - yaxis - title
    text`
)
...
setYAxisXPosition(yaxisLabelCoords, yTitleCoords)
{
    ...
    w.config.yaxis.map((yaxe, index) = > {
        let shouldNotDrawAxis =
            w.globals.ignoreYAxisIndexes.indexOf(index) > -1 || !yaxe.show ||
            yaxe.floating ||
...
setYAxisTextAlignments()
{
    ...
    let yaxis = w.globals.dom.baseEl.querySelectorAll(`.
    apexcharts - yaxis`)
