define(['app/desktop/modules/stores/store.module',
    ...
    function (module) {
        ...
        function StockReceiveItemsController($scope, $sce, $rootScope, $timeout, ObjectAttributeService, ItemService, AttributeAttachmentService, ObjectTypeAttributeService, $window, $state, $stateParams,
                                             TopStockReceivedService, ProjectService) {
            var vm = this;
            ...
            $rootScope.stockReceiveItemsList = angular.copy(pagedResults);
            ...
            function editItemAttributes(item, type) {
                ...
                var options = {
                    ...
                    data: {
                        objType: type,
                        hasPermission: $rootScope.hasPermission('permission.receives.editReceive')
                    },
            ...
            function loadReceiveItems() {
                vm.loading = true;
                TopStockReceivedService.getPagedStockReceiveItems($rootScope.storeId, $stateParams.receiveId, pageable).then(
                    function (stockMovements) {
                        $rootScope.stockReceiveItemsList = stockMovements;
                        vm.loading = false;
                        loadAttributeValues();
                    }
                );
            ...
            function showItemDetails(item) {
                item.itemDTO.receivedOn = new Date();
                item.itemDTO.quantity = item.quantity;
                item.itemDTO.type = "RECEIVEITEM";
                var options = {
                    title: 'Receive Item Details',
                    showMask: true,
            ...
            function showAttributes() {
                var options = {
                    ...
                    callback: function (result) {
                        ...
                        $window.localStorage.setItem("receiveItemAttributes", JSON.stringify(vm.objectAttributes));
                        if (result.length > 0) {
            ...
            function showRequiredImage(attribute) {
                var modal = document.getElementById('stockRecModal1');
                var modalImg = document.getElementById('stockRecImg1');
            ...
            function validateJSON() {
                try {
                    JSON.parse($window.localStorage.getItem("receiveItemAttributes"));
                } catch (e) {
                    return false;
                }
                return true;
            ...
            function loadRequiredAttributes() {
                ItemService.getTypeAttributesRequiredTrue("RECEIVEITEM").then(
                    function (data) {
            ...
            function loadAttributeValues() {
                ...
                angular.forEach($rootScope.stockReceiveItemsList.content, function (obj) {
                ...
                if (vm.objectIds.length > 0 && vm.attributeIds.length > 0) {
                    ItemService.getAttributesByObjectIdsAndAttributeIds(vm.objectIds, vm.attributeIds).then(
                        function (data) {
                            ...
                            angular.forEach($rootScope.stockReceiveItemsList.content, function (item) {
                                var attributes = [];
            ...
            function loadAttributes() {
                if (validateJSON()) {
                    var setAttributes = JSON.parse($window.localStorage.getItem("receiveItemAttributes"));
                } else {
                ...
                if (setAttributes != null && setAttributes != undefined) {
                    ...
                    ObjectTypeAttributeService.getObjectTypeAttributesByIds(vm.Ids).then(
                        function (data) {
                            if (data.length == 0) {
                                setAttributes = null;
                                $window.localStorage.setItem("receiveItemAttributes", "");
            ...
            function nextPage() {
                if ($rootScope.stockReceiveItemsList.last != true) {
                    pageable.page++;
                    loadReceiveItems();
                }
            }
            ...
            function previousPage() {
                if ($rootScope.stockReceiveItemsList.first != true) {
                    pageable.page--;
                    loadReceiveItems();
                }
            }
            ...
            $scope.$on('app.receive.items.attributes', function () {
                showAttributes();
            });
            ...
            (function () {
                if ($application.homeLoaded == true) {
                    loadCurrencies();
                    $rootScope.$on('app.stock.receiveItems', function (event, data) {
                        loadRequiredAttributes();
                        //resize();
                    });
                    $scope.$on('app.stores.receiveItems.nextPageDetails', nextPage);
                    $scope.$on('app.stores.receiveItems.previousPageDetails', previousPage);
                }
            })();
        }
    }
