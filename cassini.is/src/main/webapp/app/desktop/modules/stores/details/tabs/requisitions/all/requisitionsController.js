...
define(['app/desktop/modules/stores/store.module',
    ...
    function (module) {
        ...
        function RequestsController($scope, $rootScope, $window, $state, $stateParams, AttributeAttachmentService, ObjectTypeAttributeService, RequisitionService, TopStoreService, ItemService, ObjectAttributeService, $sce) {
            var vm = this;

            vm.requisitions = [];
            vm.requiredRequisitionAttributes = [];
            $rootScope.storeRequisitionList = [];
            var currencyMap = new Hashtable();
            vm.objectIds = [];

            vm.showRequisitionDetails = showRequisitionDetails;
            ...
            $rootScope.storeRequisitionList = pagedResults;
            vm.requisitions = pagedResults;
            ...
            function loadRequisitions() {
                vm.loading = true;
                RequisitionService.getPageableRequisitions($rootScope.storeId, pageable).then(
                    function (data) {
                        vm.loading = false;
                        ...
                        $rootScope.storeRequisitionList = data;
                        loadAttributeValues();
                    }
                )
            }

            function showRequisitionDetails(requsition) {
                $rootScope.requisitionId = requsition.id;
                $state.go('app.store.stock.requisitionDetails', {requisitionId: requsition.id})
            }
            ...
            function showRequiredImage(attribute) {
                var modal = document.getElementById('reqModal1');
                var modalImg = document.getElementById('reqImg1');
            ...
            function showImage(attribute) {
                ...
                var modalImg = document.getElementById('reqImg');
            ...
            function loadRequiredAttributes() {
                ItemService.getTypeAttributesRequiredTrue("CUSTOM_REQUISITION").then(
                    function (data) {
            ...
            function showAttributes() {
                var options = {
                    ...
                    data: {
                        ...
                        attributesMode: 'CUSTOM_REQUISITION'
                    },
                    buttons: [
            ...
            function loadAttributeValues() {
                ...
                angular.forEach(vm.requisitions.content, function (obj) {
                ...
                if (vm.objectIds.length > 0 && vm.attributeIds.length > 0) {
                    ItemService.getAttributesByObjectIdsAndAttributeIds(vm.objectIds, vm.attributeIds).then(
                        function (data) {
                            ...
                            angular.forEach(vm.requisitions.content, function (item) {
                                var attributes = [];
            ...
            (function () {
                $scope.$on('app.store.tabactivated', function (event, data) {
                    if (data.tabId == 'details.requests') {
                        loadStore();
                        loadCurrencies();
                        loadAttributes();
                    }
                ...
                $scope.$on('app.stores.requisitions.nextPageDetails', nextPage);
                $scope.$on('app.stores.requisitions.previousPageDetails', previousPage);
            })();
            $scope.$on('app.Store.requisition.attributes', function () {
                showAttributes();
            })
        }
    }
