...
<div class="responsive-table">
    <table class="table table-striped">
        ...
        <tr>
            ...
            <th style="min-width: 150px; text-align: center">Qty</th>
            <th style="min-width: 150px">Notes</th>
            <th style="min-width: 150px" ng-repeat="reqObjectAttribute in roadItemsVm.requiredObjectAttributes">
                {{reqObjectAttribute.name}}
            </th>

            <th style="min-width: 150px" class='added-column'
                ng-repeat="objectAttribute in roadItemsVm.objectAttributes">
                {{objectAttribute.name}}
                <i class="fa fa-times-circle" style="cursor: pointer;"
                   ng-click="roadItemsVm.removeAttribute(objectAttribute)"
        ...
        <tbody>
        <tr ng-if="storeRoadChallanItemsList.content.length == 0 && roadItemsVm.loading == false">
            <td colspan="25">No Items are available to view</td>
        </tr>
        <tr ng-if="roadItemsVm.loading == true">
            <td colspan="25">
        ...
        <tr data-ng-repeat="item in storeRoadChallanItemsList.content">
            <td style="width: 150px; text-align: left">
                <span><%--<a href="" ng-click="roadItemsVm.showItemDetails(item)" title="Click to show Details">--%>{{item.materialItem.itemNumber}}<%--</a>--%></span>

            </td>
            <td title="{{item.materialItem.itemName}}">{{item.materialItem.itemName | limitTo:
                15}}{{item.materialItem.itemName.length > 15 ? '...' : ''}}
            </td>

            <td title="{{item.materialItem.description}}">{{item.materialItem.description | limitTo:
                15}}{{item.materialItem.description.length > 15 ? '...' : ''}}
            </td>
            ...
            <td title="{{item.notes}}">{{item.notes | limitTo: 15}}{{item.notes.length > 15 ? '...' : ''}}</td>
            <%-- adding attribute values--%>
            <td class="added-column" ng-repeat="objectAttribute in roadItemsVm.requiredObjectAttributes">
                    ...
                    <div class="attributeTooltiptext">
                        <ul>
                            <li ng-repeat="attachment in item[attrName]">
                                <a href="" ng-click="roadItemsVm.openAttachment(attachment)"
                ...
                <div ng-if="objectAttribute.name != 'modifiedBy' && objectAttribute.name != 'createdBy'
                     ...
                     ng-init="attrName = objectAttribute.name">
                    <a href="" ng-click="roadItemsVm.showImage(item[attrName])"
                    ...
                    <div id="myModal2" class="img-model modal" style="z-index: 11">
            ...
            <td class="added-column" ng-repeat="objectAttribute in roadItemsVm.objectAttributes">
                ...
                <div ng-if="objectAttribute.name != 'modifiedBy' && objectAttribute.name != 'createdBy'
                                        && objectAttribute.dataType != 'CURRENCY' && objectAttribute.dataType == 'IMAGE'"
                     ng-init="attrName = objectAttribute.name">
                    <a href="" ng-click="roadItemsVm.showRequiredImage(item[attrName])"
                       title="Click to show large Image">
                        <img ng-if="item[attrName] != null"
                             ng-src="{{item[attrName]}}"
                             style="height: 30px;width: 40px;margin-bottom: 5px;">
                    ...
                    <div id="image" class="img-model modal" style="z-index: 11">
                        <span class="closeImage2" style="margin-left: 85%;">&times;</span>
            ...
            <td style="text-align: center;">
                <i ng-if="!item.itemDTO.editAttribute && hasPermission('permission.roadChallans.edit')"
                   title="addAttribute" style="color:#5e6258; font-size: 15px;" class="fa fa-pencil"
                   ng-click="roadItemsVm.showItemAttributes(item,'ROADCHALLANITEM')" aria-hidden="true"></i>
                <i ng-if="item.itemDTO.editAttribute" style="color: #252721; font-size: 15px;" title="editAttribute"
                   class="fa fa-pencil" ng-click="roadItemsVm.editItemAttributes(item,'ROADCHALLANITEM')"
                   aria-hidden="true"></i>
            </td>
