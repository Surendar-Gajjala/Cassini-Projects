...
define(
    ...
    function (module) {
        module.controller('LoanBasicController', LoanBasicController);

        function LoanBasicController($scope, $rootScope, $timeout, $state, $stateParams,
                                     LoanService, CommonService, ProjectService, TopStoreService) {
            ...
            function loadLoan() {
                LoanService.getLoanById($rootScope.storeId, $stateParams.loanId).then(
                    function (data) {
                        vm.loan = data;
                        $rootScope.viewInfo.title = "Loan Issue :" + vm.loan.loanNumber;
                        vm.loading = false;
                        CommonService.getPerson(vm.loan.createdBy).then(
                            ...
                            ProjectService.getProject(vm.loan.fromProject).then(
                                ...
                                TopStoreService.getTopStore(vm.loan.toStore).then(
                                    function (store) {
                                        vm.loan.toStore = store.storeName;
                                    }
                                )
                            )
                        );
