define(['app/desktop/modules/stores/store.module',
    ...
    function (module) {
        module.controller('AllScrapRequestController', AllScrapRequestController);

        function AllScrapRequestController($scope, $rootScope, $sce, ObjectAttributeService, TopStoreService, ObjectTypeAttributeService, AttributeAttachmentService, $window, $state, BomService, ScrapService, CommonService, ProjectService, ItemService) {
            ...
            vm.showScrapRequestAttributes = showScrapRequestAttributes;
            ...
            function showDetails(scrap) {
                $rootScope.scrapDetailsId = scrap.id;
                $state.go('app.store.scrapDetails', {scrapDetailsId: scrap.id});
            }
            ...
            function loadScrap() {
                ScrapService.getAll(vm.pageable).then(
                    function (data) {
                        vm.scraps = data;
                        angular.forEach(vm.scraps.content, function (scrap) {
                            ProjectService.getProject(scrap.project).then(
                                function (project) {
                                    scrap.project = project.name;
                                }
                            )
                            TopStoreService.getTopStore(scrap.store).then(
                                function (store) {
                                    scrap.store = store.storeName;
                                }
                            )
                            CommonService.getPerson(scrap.requestedBy).then(
                                function (person) {
                                    scrap.requestedBy = person.firstName;
                                }
                            )
                        });
                        loadAttributeValues();
            ...
            function loadRequiredAttributes() {
                ItemService.getTypeAttributesRequiredTrue("SCRAPREQUEST").then(
                    function (data) {
            ...
            function showScrapRequestAttributes() {
                var options = {
                    title: 'Scrap Request Attributes',
                    showMask: true,
                    ...
                    callback: function (result) {
                        ...
                        $window.localStorage.setItem("scrpAttibutes", JSON.stringify(vm.objectAttributes));
                        if (result.length > 0) {
            ...
            function showImage(attribute) {
                var modal = document.getElementById('scrapModal');
                var modalImg = document.getElementById('scrapImg');
            ...
            function loadAttributeValues() {
                ...
                angular.forEach(vm.scraps.content, function (obj) {
                ...
                if (vm.objectIds.length > 0 && vm.attributeIds.length > 0) {
                    ItemService.getAttributesByObjectIdsAndAttributeIds(vm.objectIds, vm.attributeIds).then(
                        function (data) {
                            ...
                            angular.forEach(vm.scraps.content, function (item) {
                                var attributes = [];
                                    ...
                                    if (selectatt != null) {
                                        ...
                                        if (selectatt.dataType == 'TEXT') {
                                            ...
                                            if (attribute.refValue != null) {
                                                var objectSelector = $application.getObjectSelector(selectatt.refType);
                                                if (objectSelector != null && attribute.refValue != null) {
                                                    objectSelector.getDetails(attribute.refValue, selectatt, function (displayValue) {
                                                        item[attributeName] = displayValue;
                                                    });
                                                }
                                            }
                                        }
            ...
            function validateJSON() {
                try {
                    JSON.parse($window.localStorage.getItem("scrpAttibutes"));
                } catch (e) {
                    return false;
                }
                return true;
            ...
            function loadAttributes() {
                if (validateJSON()) {
                    var setAttributes = JSON.parse($window.localStorage.getItem("scrpAttibutes"));
                } else {
                ...
                if (setAttributes != null && setAttributes != undefined) {
                    ...
                    ObjectTypeAttributeService.getObjectTypeAttributesByIds(vm.Ids).then(
                        function (data) {
                            if (data.length == 0) {
                                setAttributes = null;
                                $window.localStorage.setItem("scrpAttibutes", "");
            ...
            (function () {
                ...
                if ($application.homeLoaded == true) {
                    loadAttributes();
                    resize();
                    $scope.$on('app.store.scrapRequest.attributes', function () {
                        showScrapRequestAttributes();
                    })
                }
            })();
