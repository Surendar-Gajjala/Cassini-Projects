define(
    ...
    function (module) {
        module.controller('MachineTypeDetailsController', MachineTypeDetailsController);

        function MachineTypeDetailsController($scope, $rootScope, $timeout, $state, $stateParams, $cookies, ItemService, ItemTypeService,
            ...
            function machineTypeSelected(event, args) {
                vm.machineType = args.typeObject;
                machineName = args.typeObject.name;
                nodeId = args.nodeId;
                if (vm.machineType != null && vm.machineType != undefined) {
                    if (vm.machineType.id == undefined || vm.machineType.id == null) {
                        vm.attributesShow = false;
                        $scope.$apply();
                    } else {
            ...
            function loadAttributes() {
                if (vm.machineType.id != null || vm.machineType.id != undefined) {
                    ItemTypeService.getMachineAttributesWithHierarchy(vm.machineType.id).then(
                        function (data) {
                            ...
                            vm.machineType.attributes = data;
                        }
                    );
                }
            }
                ...
                AutonumberService.getAutonumbers().then(
                    function (data) {
                        ...
                        angular.forEach(data, function (item) {
                            if (vm.machineType.machineNumberSource == null && item.name == "Default Machine Item Number Source") {
                                vm.machineType.machineNumberSource = item;
                                if (vm.machineType.id != undefined) {
                                    ClassificationService.updateType('MACHINETYPE', vm.machineType).then(
                                        function (data) {
                                        }
                                    );
                                }
            ...
            function onSave() {
                if (vm.machineType != null && validate()) {

                    if (vm.machineType.id == null || vm.machineType.id == undefined) {

                        ClassificationService.createType('MACHINETYPE', vm.machineType).then(
                            function (data) {
                                if (data != null) {
                                    $rootScope.showSuccessMessage(data.name + " : created successfully");
                                    vm.attributesShow = true;
                                    vm.machineType.id = data.id;
                                    $rootScope.$broadcast("app.classification.update", {
                                        nodeId: nodeId,
                                        nodeName: vm.machineType.name
                                    });
                                }
                                else if (data == null) {
                                    $rootScope.showErrorMessage(vm.machineType.name + " : already exists");
                                    vm.machineType.name = machineName;
                                }
                            },
                            function (error) {
                                $rootScope.showErrorMessage(vm.machineType.name + " : already exists");
            ...
            function validate() {
                ...
                if (vm.machineType.name == null || vm.machineType.name == "") {
                    vm.valid = false;
                    $rootScope.showErrorMessage("Machine Type Name cannot be empty");
                }
            ...
            function addAttribute() {
                ...
                att.itemType = vm.machineType.id;
                vm.machineType.attributes.unshift(att);
            }
                ...
                if (attribute.id == null || attribute.id == undefined) {
                    if (validateAttribute(attribute)) {
                        if (attributeMap.get(attribute.newName) != null) {
                            ...
                            vm.machineType.attributes.splice(vm.machineType.attributes.indexOf(attribute), 1);
                        } else {
                            ...
                            promise = ClassificationService.createTypeAttribute('MACHINETYPE', vm.machineType.id, attribute);
                    ...
                    if (attributeMap.get(attribute.newName) == null || (attributeMap.get(attribute.newName) != null && attributeMap.get(attribute.newName).id == attribute.id)) {
                        ...
                        attribute.refType = attribute.newRefType;
                        attribute.lov = attribute.newLov;
                        promise = ClassificationService.updateTypeAttribute('MACHINETYPE', vm.machineType.id, attribute);
            ...
            function deleteAttribute(attribute) {
                ...
                ItemService.findMachineItemsByAttribute(attribute.id).then(
                    function (data) {
                        if (data.length == 0) {
                            var options = {
                            ...
                            DialogService.confirm(options, function (yes) {
                                if (yes == true) {
                                    if (attribute.id != null && attribute.id != undefined) {
                                        ClassificationService.deleteTypeAttribute('MACHINETYPE', vm.machineType.id, attribute.id).then(
                                            function (data) {
                                                vm.machineType.attributes.splice(vm.machineType.attributes.indexOf(attribute), 1);
            ...
            (function () {
                $scope.$on('app.machineType.selected', machineTypeSelected);
                $scope.$on('app.machineType.save', onSave);
            })();
        }
    }
);