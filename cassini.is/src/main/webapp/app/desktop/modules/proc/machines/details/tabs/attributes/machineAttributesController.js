define(['app/desktop/modules/proc/proc.module',
    ...
    function (module) {
        ...
        function MachineAttributesController($scope, $rootScope, $timeout, $sce, $state, $stateParams, $cookies, DialogService, ObjectAttributeService, AttachmentService, ItemTypeService, CommonService, AttributeAttachmentService, ItemService, ItemTypeService) {
            var vm = this;
            ...
            vm.saveMachineAttribute = saveMachineAttribute;
            ...
            function loadAttributeDefs() {
                vm.attributes = [];
                ItemTypeService.getMachineAttributesWithHierarchy(vm.machine.itemType.id).then(
                    function (data) {
                        vm.loading = false;
            ...
            function loadAttributes() {
                ItemService.getMachineItemAttributes(vm.machineId).then(
                    function (data) {
            ...
            function saveMachineAttribute(attribute) {
                if (attribute.value.id == undefined || attribute.value.id == null) {
                    ItemService.saveMachineItemAttributes(vm.machine.id, attribute.value).then(
                        function (data) {
                    ...
                    ItemService.updateMachineItemAttributes(vm.machine.id, attribute.value).then(
                        function (data) {
                            attribute.listValueEditMode = false;
                            attribute.changeCurrency = false;
                            attribute.editMode = false;
            ...
            function saveTime(attribute) {
                if (attribute.editTimeValue != null && attribute.attributeDef.dataType == 'TIME') {
                    ...
                    if (attribute.attributeDef.objectType == "MACHINETYPE") {
                        ItemService.updateMachineItemAttributes(vm.machine.id, attribute.value).then(
                            function (data) {
                                attribute.showTimeAttribute = false;
            ...
            function saveImage(attribute) {
                ...
                if (attribute.newImageValue != null) {
                    ...
                    ItemService.updateMachineItemAttributes(vm.machineId, attribute.value).then(
                        function (data) {
            ...
            function saveAttachments(attribute) {
                ...
                if (attribute.attachmentValues.length > 0) {
                    ...
                    angular.forEach(attribute.attachmentValues, function (attachmentValue) {
                        ...
                        angular.forEach(attachments, function (machineAttachment) {
                            if ((attribute.id.objectId == machineAttachment.objectId) && (attribute.id.attributeDef == machineAttachment.attributeDef) && (attachmentValue.name == machineAttachment.name)) {
                                machineAttachmentExists = true;
                                attachmentIds.remove(machineAttachment.id);
                                ...
                                DialogService.confirm(options, function (yes) {
                                    if (yes == true) {
                                        $rootScope.showBusyIndicator($(".view-content"));
                                        AttributeAttachmentService.saveAttributeAttachment(attribute.id.objectId, attribute.id.attributeDef, 'MACHINE', attachmentValue).then(
                                            function (data) {
            ...
            /*-------------- To Display Large Machine Attribute Image -----------------*/
