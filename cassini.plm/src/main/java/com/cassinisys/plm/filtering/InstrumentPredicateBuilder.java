...
import com.cassinisys.plm.model.mes.QMESInstrument;
 ...
 */
@Component
public class InstrumentPredicateBuilder implements PredicateBuilder<InstrumentCriteria, QMESInstrument> {
    ...
    @Override
    public Predicate build(InstrumentCriteria instrumentCriteria, QMESInstrument pathBase) {
        return getDefaultPredicate(instrumentCriteria, pathBase);

    }

    private Predicate getDefaultPredicate(InstrumentCriteria criteria, QMESInstrument pathBase) {
        ...
        if (!Criteria.isEmpty(criteria.getInstrumentType())) {
            predicates.add(pathBase.type.id.eq(criteria.getInstrumentType()));
            if (!Criteria.isEmpty(criteria.getBopRoute())) {
                ...
                List<Integer> ids = bopOperationResourceRepository.getResourceIdsByPlanAndOperationAndResourceType(criteria.getBopRoute(), plan.getOperation(), criteria.getInstrumentType());
                if (ids.size() > 0) {
