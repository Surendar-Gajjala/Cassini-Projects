...
import com.cassinisys.plm.model.cm.QPLMManufacturerMCO;
 ...
 */
@Component
public class ManufacturerMCOPredicateBuilder implements PredicateBuilder<ManufacturerMCOCriteria, QPLMManufacturerMCO> {

    @Autowired
    private PLMWorkflowService workflowService;

    @Override
    public Predicate build(ManufacturerMCOCriteria criteria, QPLMManufacturerMCO pathBase) {
        return getDefaultPredicate(criteria, pathBase);
    }

    private Predicate getDefaultPredicate(ManufacturerMCOCriteria criteria, QPLMManufacturerMCO pathBase) {
        ...
        if (!Criteria.isEmpty(criteria.getStatus())) {
            List<Integer> ids = workflowService.getWorkflowAttachedToIdsByStatusAndType(criteria.getStatus(), PLMObjectType.OEMPARTMCO);
