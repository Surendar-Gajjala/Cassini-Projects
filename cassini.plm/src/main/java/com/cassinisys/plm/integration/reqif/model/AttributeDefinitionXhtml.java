package com.cassinisys.plm.integration.reqif.model;

import org.jdom2.Element;

public class AttributeDefinitionXhtml extends AttributeDefinition {

    public AttributeDefinitionXhtml(DatatypeDefinition type) {
        super(type);
    }

    @Override
    public Element toXml() {
        Element elem = createXmlElement("ATTRIBUTE-DEFINITION-XHTML");
        Element type = new Element("TYPE");
        Element ref = new Element("DATATYPE-DEFINITION-XHTML-REF");
        ref.setText(getType().getIdentifier());
        type.addContent(ref);
        elem.addContent(type);
        return elem;
    }

    @Override
    public Element createRefElement() {
        Element elem = new Element("ATTRIBUTE-DEFINITION-XHTML-REF");
        elem.setText(getIdentifier());
        return elem;
    }
}
