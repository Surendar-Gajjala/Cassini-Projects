define(
    ...
    function (module) {
        module.directive('itemClassTree', ['$compile', '$timeout', 'ItemTypeService', function ($compile, $timeout, ItemTypeService) {
            return {
                templateUrl: 'app/desktop/modules/classification/directive/itemClassTreeDirective.jsp',
                restrict: 'E',
                replace: false,
                scope: {
                    'treeId': '@',
                ...
                link: function ($scope, elm, attr) {
                    if ($scope.treeId == null || $scope.treeId === undefined || $scope.treeId === '') $scope.treeId = "itemClassTree";
                    ...
                    function initClassificationTree() {
                        classificationTree = $('#' + $scope.treeId).tree({
                            data: [
                                {
                                    id: nodeId,
                                    text: 'Classification',
                    ...
                    function loadClassificationTree() {
                        var promise = null;
                        if ($scope.itemClass != null && $scope.itemClass != "" && $scope.itemClass != undefined) {
                            promise = ItemTypeService.getItemClassTypeTree($scope.itemClass);
                        } else {
                            promise = ItemTypeService.getClassificationTree();
                        }
