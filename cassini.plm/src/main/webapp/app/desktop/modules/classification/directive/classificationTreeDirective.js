define(
    [
        'app/desktop/desktop.app',
        'jquery-ui',
        'app/shared/services/core/itemTypeService'
    ],

    function (module) {
        module.directive('classificationTree', ['$compile', '$timeout', 'ItemTypeService', function ($compile, $timeout, ItemTypeService) {
            return {
                templateUrl: 'app/desktop/modules/classification/directive/classificationTreeDirective.jsp',
                restrict: 'E',
                replace: false,
                scope: {
                    'treeId': '@',
                    'onSelectType': '='
                },

                link: function ($scope, elm, attr) {
                    if($scope.treeId == null || $scope.treeId === undefined || $scope.treeId === '') $scope.treeId = "classificationTree";
                    var nodeId = 0;
                    var classificationTree = null;
                    var rootNode = null;

                    function onSelectType(node) {
                        var data = classificationTree.tree('getData', node.target);
                        var itemType = data.attributes.itemType;
                        $scope.onSelectType(itemType);
                        window.$("body").trigger("click");
                    }

                    function initClassificationTree() {
                        classificationTree = $('#' + $scope.treeId).tree({
                            data: [
                                {
                                    id: nodeId,
                                    text: 'Classification',
                                    iconCls: 'classification-root',
                                    attributes: {
                                        itemType: null
                                    },
                                    children: []
                                }
                            ],
                            onSelect: onSelectType
                        });

                        rootNode = classificationTree.tree('find', 0);
                    }

                    function loadClassificationTree() {
                        ItemTypeService.getItemTypeTree().then(
                            function (data) {
                                var nodes = [];
                                angular.forEach(data, function (itemType) {
                                    var node = makeNode(itemType);

                                    if (itemType.children != null && itemType.children != undefined && itemType.children.length > 0) {
                                        node.state = "closed";
                                        visitChildren(node, itemType.children);
                                    }

                                    nodes.push(node);
                                });

                                classificationTree.tree('append', {
                                    parent: rootNode.target,
                                    data: nodes
                                });
                            }, function (error) {
                                  $rootScope.showErrorMessage(error.message);
                                  $rootScope.hideBusyIndicator();
                             }
                        )
                    }

                    function visitChildren(parent, itemTypes) {
                        var nodes = [];
                        angular.forEach(itemTypes, function (itemType) {
                            var node = makeNode(itemType);

                            if (itemType.children != null && itemType.children != undefined && itemType.children.length > 0) {
                                node.state = 'closed';
                                visitChildren(node, itemType.children);
                            }

                            nodes.push(node);
                        });

                        if (nodes.length > 0) {
                            parent.children = nodes;
                        }
                    }

                    function makeNode(itemType) {
                        return {
                            id: ++nodeId,
                            text: itemType.name,
                            iconCls: 'itemtype-node',
                            attributes: {
                                itemType: itemType
                            }
                        };
                    }

                    $timeout(function() {
                        initClassificationTree();
                        loadClassificationTree();
                    }, 1000);
                }
            };
        }]);
    }
);
