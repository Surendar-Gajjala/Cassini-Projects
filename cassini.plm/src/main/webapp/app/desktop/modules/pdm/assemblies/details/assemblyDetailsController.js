define(
    [
        'app/desktop/modules/pdm/pdm.module',
        'app/desktop/modules/pdm/assemblies/details/tabs/basic/basicController',
        'app/desktop/modules/pdm/assemblies/details/tabs/drawing/drawingController',
        'app/desktop/modules/pdm/assemblies/details/tabs/timeline/timelineController',
        'app/desktop/modules/pdm/assemblies/details/tabs/visualization/visualizationController',
        'app/shared/services/core/pdmService',
        'app/shared/services/core/pdmVaultService'
    ],
    function (module) {
        module.controller('AssemblyDetailsController', AssemblyDetailsController);

        function AssemblyDetailsController($scope, $rootScope, $timeout, $state, $stateParams, $sce, $cookies, $window,
                                           PDMService, PDMVaultService) {

            $rootScope.viewInfo.showDetails = false;

            var vm = this;

            vm.assemblyId = $stateParams.assemblyId;
            vm.assembly = null;
            vm.assemblyBom = null;
            vm.expanded = false;
            vm.selectedItem = null;
            vm.selectedFileVersion = null;
            vm.bomItems = [];
            vm.vault = null;
            vm.folder = null;
            vm.breadCrumbs = null;
            vm.loading = true;
            vm.fileVersions = [];
            $scope.fileVersions = [];

            vm.tabs = {
                basic: {
                    id: 'details.basic',
                    heading: "Basic",
                    template: 'app/desktop/modules/pdm/assemblies/details/tabs/basic/basicView.jsp',
                    controller: "AssemblyDetailsBasicController as basicVm",
                    index: 0,
                    active: true,
                    activated: true
                },
                visualization: {
                    id: 'details.visualization',
                    heading: "Visualization",
                    template: 'app/desktop/modules/pdm/assemblies/details/tabs/visualization/visualizationView.jsp',
                    controller: "AssemblyDetailsVisualizationController as visualizationVm",
                    index: 1,
                    active: false,
                    activated: false
                },
                drawing: {
                    id: 'details.drawing',
                    heading: "Drawing",
                    template: 'app/desktop/modules/pdm/assemblies/details/tabs/drawing/drawingView.jsp',
                    controller: "AssemblyDrawingController as drawingVm",
                    index: 1,
                    active: false,
                    activated: false
                },
                timeline: {
                    id: 'details.timeline',
                    heading: "History",
                    template: 'app/desktop/modules/pdm/assemblies/details/tabs/timeline/timelineView.jsp',
                    controller: "TimelineController as timelineVm",
                    index: 2,
                    active: false,
                    activated: false
                }
            };
            vm.activeTab = 0;


            function loadAssembly() {
                vm.bomItems = [];
                PDMService.getAssembly(vm.assemblyId).then(
                    function (data) {
                        vm.assembly = data;
                        vm.selectedItem = vm.assembly;
                        $scope.selectedItem = vm.selectedItem;
                        vm.selectedItem.selected = true;
                        $scope.$broadcast('app.assembly.bomitem.selected', {selectedItem: vm.assembly});

                        loadBom();
                        loadFileVersion();
                    }, function (error) {
                        $rootScope.showErrorMessage(error.message);
                        $rootScope.hideBusyIndicator();
                    }
                );
            }

            vm.chilrenCount = 0;
            function loadBom() {
                PDMService.getAssemblyBom(vm.assemblyId).then(
                    function (data) {
                        data.level = 0;
                        angular.forEach(data.children, function (bomItem) {
                            bomItem.level = 1;
                            bomItem.expanded = false;
                            bomItem.selected = false;
                            /*if (bomItem.children != null && bomItem.children != undefined) {
                             if (bomItem.children.length > 0) {
                             vm.chilrenCount++;
                             }
                             }*/
                            vm.bomItems.push(bomItem);
                        });
                        vm.assemblyBom = data;
                    }, function (error) {
                        $rootScope.showErrorMessage(error.message);
                        $rootScope.hideBusyIndicator();
                    }
                );
            }

            vm.toggleBomItem = toggleBomItem;
            function toggleBomItem(event, assembly) {
                event.preventDefault();
                event.stopPropagation();

                var children = assembly.children;
                var index = vm.bomItems.indexOf(assembly);
                if (assembly.expanded) {
                    removeChildren(assembly);
                }
                else {
                    angular.forEach(children, function (child) {
                        index = index + 1;
                        child.level = assembly.level + 1;
                        vm.bomItems.splice(index, 0, child);
                    });
                }
                assembly.expanded = !assembly.expanded;
                var childCount = 0;
                var expandedCount = 0;
                angular.forEach(vm.bomItems, function (item) {
                    if (item.children != null && item.children != undefined) {
                        if (item.children.length > 0) {
                            childCount++;
                            if (!item.expanded) {
                                vm.expanded = false;
                            } else {
                                expandedCount++;
                            }
                        }
                    }
                });
                if (childCount == expandedCount) {
                    vm.expanded = true;
                }


            }

            function removeChildren(bomItem) {
                if (bomItem != null && bomItem.children != null && bomItem.children !== undefined) {
                    angular.forEach(bomItem.children, function (item) {
                        if (item.expanded) {
                            removeChildren(item);
                        }

                        item.expanded = false;
                    });
                    /*if (vm.chilrenCount == 1) {
                     vm.expanded = false;
                     }*/
                    var index = vm.bomItems.indexOf(bomItem);
                    vm.bomItems.splice(index + 1, bomItem.children.length);
                }
            }

            vm.toggleAllItems = toggleAllItems;
            function toggleAllItems() {
                if (vm.expanded) {
                    angular.forEach(vm.bomItems, function (bomItem) {
                        bomItem.expanded = false
                    });

                    vm.bomItems = [];
                    angular.forEach(vm.assemblyBom.children, function (bomItem) {
                        bomItem.level = 1;
                        bomItem.expanded = false;
                        bomItem.selected = false;
                        vm.bomItems.push(bomItem);
                    });
                }
                else {
                    vm.bomItems = [];
                    expandChildren(vm.assemblyBom);
                }
                vm.expanded = !vm.expanded;
            }

            function expandChildren(assembly) {
                angular.forEach(assembly.children, function (bomItem) {
                    bomItem.level = assembly.level + 1;
                    bomItem.expanded = true;
                    vm.bomItems.push(bomItem);
                    if (bomItem['@type'] === 'AssemblyDTO') {
                        expandChildren(bomItem);
                    }
                });
            }

            $scope.selectedItem = null;
            vm.loadSelectedItem = loadSelectedItem;
            function loadSelectedItem(item) {
                if (vm.selectedItem != null) {
                    vm.selectedItem.selected = false;
                }
                vm.selectedItem = item;
                vm.selectedItem.selected = true;
                $scope.selectedItem = vm.selectedItem;

                $scope.$broadcast('app.assembly.bomitem.selected', {selectedItem: vm.selectedItem});
            }


            function loadVault() {
                PDMVaultService.getVault(vm.selectedFileVersion.file.vault).then(
                    function (data) {
                        vm.vault = data;
                        loadFolder();
                    }, function (error) {
                        //$rootScope.showErrorMessage(error.message);
                        $rootScope.hideBusyIndicator();
                    }
                )
            }

            function loadFolder() {
                PDMVaultService.getFolder(vm.selectedFileVersion.file.vault, vm.selectedFileVersion.file.folder).then(
                    function (data) {
                        vm.folder = data;
                        vm.breadCrumbs = getBreadCrumbsLink(vm.vault, vm.folder);
                        vm.loading = false;
                    }, function (error) {
                        $rootScope.showErrorMessage(error.message);
                        $rootScope.hideBusyIndicator();
                    }
                )
            }

            function loadFileVersion() {
                PDMService.getFileVersionForRevisionedObject(vm.assembly.id).then(
                    function (data) {
                        vm.selectedFileVersion = data;
                        loadVault();
                        loadFileVersions();
                    }, function (error) {
                        $rootScope.showErrorMessage(error.message);
                        $rootScope.hideBusyIndicator();
                    }
                );
            }

            function getBreadCrumbsLink(vault, folder) {
                var link = "";

                var ngClick = "goToAllVaults()";
                link += "<a href='javascript:void(0);' onclick=\"" + ngClick + "\">" + "All Vaults" + "</a>";
                link += " &raquo; ";

                ngClick = "openVaultFolder(null)";
                link += "<a href='javascript:void(0);' onclick=\"" + ngClick + "\">" + vault.name + "</a>";

                var idPath = folder.idPath;
                var namePath = folder.namePath;

                if (idPath != null && namePath != null) {
                    link += " &raquo; ";
                    var arrId = idPath.split('/');
                    var arrName = namePath.split('/');

                    if (arrId.length === arrName.length) {
                        var path = "";

                        for (var i = 0; i < arrName.length; i++) {
                            path += arrId[i];
                            ngClick = "openVaultFolder('" + path + "')";
                            link += "<a href='javascript:void(0);' onclick=\"" + ngClick + "\">" + arrName[i] + "</a>";

                            if (i !== arrName.length - 1) {
                                link += " &raquo; ";
                                path += "/";
                            }
                        }
                    }
                }

                return $sce.trustAsHtml(link);
            }

            $window.openVaultFolder = openFolder;
            function openFolder(path) {
                $state.go('app.pdm.vaultdetails', {
                    vaultId: vm.vault.id, path: path
                }, {notify: true});
            }

            $window.goToAllVaults = goToAllVaults;
            function goToAllVaults() {
                $state.go('app.pdm.vaults', {}, {notify: true});
            }

            vm.tabSelected = tabSelected;
            $scope.selectedTab = vm.tabs.basic;
            function tabSelected(id) {
                if (id === 'details.visualization') {
                    $scope.selectedTab = vm.tabs.visualization;
                    $scope.$broadcast("visualization.tab.selected");
                }
                else if (id === 'details.basic') {
                    $scope.selectedTab = vm.tabs.basic;
                }
                else if (id === 'details.drawing') {
                    $scope.selectedTab = vm.tabs.drawing;
                    $scope.$broadcast("drawing.tab.selected");
                }
                else if (id === 'details.timeline') {
                    $scope.selectedTab = vm.tabs.timeline;
                    $scope.$broadcast("timeline.tab.selected");
                }
            }

            vm.assignPartNumbersToAssembly = assignPartNumbersToAssembly;
            function assignPartNumbersToAssembly() {
                $rootScope.showBusyIndicator();
                PDMService.assignPartNumbersToAssembly(vm.assemblyId).then(
                    function (data) {
                        loadAssembly();
                        $rootScope.showSuccessMessage("Part numbers assigned to all items");
                        $rootScope.hideBusyIndicator();

                        createEBOM();
                    },
                    function (error) {
                        $rootScope.showErrorMessage(error.message);
                        $rootScope.hideBusyIndicator();
                    }
                );
            }

            vm.createEBOM = createEBOM;
            function createEBOM() {
                $rootScope.showBusyIndicator();
                PDMService.createEBOM(vm.assemblyId).then(
                    function (data) {
                        $rootScope.hideBusyIndicator();
                        $rootScope.showSuccessMessage("EBOM created successfully");
                    },
                    function (error) {
                        $rootScope.showErrorMessage(error.message);
                        $rootScope.hideBusyIndicator();
                    }
                );
            }

            function loadFileVersions() {
                PDMVaultService.getFileVersions(vm.selectedFileVersion.file.id).then(
                    function (data) {
                        vm.fileVersions = data;
                        /*vm.fileVersions.sort(function (f1, f2) {
                            return f2.version - f1.version;
                         });*/

                        $scope.fileVersions = vm.fileVersions;
                    },
                    function (error) {
                        $rootScope.showErrorMessage(error.message);
                    }
                );
            }


            (function () {
                loadAssembly();
            })();
        }
    }
);