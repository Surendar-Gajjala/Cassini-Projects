define(
    ...
    function (module) {
        ...
        module.controller('NewSubstanceController', NewSubstanceController);

        function NewSubstanceController($scope, $q, $rootScope, $translate, $timeout, $state, $stateParams, $cookies, $sce, CommonService,
                                        AutonumberService, PGCObjectTypeService, SubstanceService, QualityTypeService, ObjectTypeAttributeService, ClassificationService) {
            ...
            var subNumberValidation = parsed.html($translate.instant("NUMBER_CANNOT_BE_EMPTY")).html();
            var subTypeValidation = parsed.html($translate.instant("ITEM_TYPE_VALIDATION")).html();
            var subNameValidation = parsed.html($translate.instant("NAME_VALIDATION")).html();
            var casNumberValidation = parsed.html($translate.instant("CAS_NUMBER_VALIDATION")).html();
            var subCreatedMsg = parsed.html($translate.instant("SUBSTANCE_CREATED_MSG")).html();
            ...
            function onSelectType(objectType) {
                if (objectType != null && objectType != undefined) {
                    vm.newSubstance.type = objectType;
                    vm.type = objectType;
                    autoNumber();
                }
            }
            ...
            function autoNumber() {
                if (vm.newSubstance.type != null && vm.newSubstance.type.autoNumberSource != null) {
                    var source = vm.newSubstance.type.autoNumberSource;
                    AutonumberService.getNextNumberByName(source.name).then(
                        function (data) {
                            vm.newSubstance.number = data;
                            loadAttributeDefs();
                        }, function (error) {
            ...
            function create() {
                if (validate()) {
                    ...
                    vm.newSubstance.pgcObjectAttributes = vm.attributes;
                    vm.newSubstance.objectAttributes = vm.customAttributes;
                    SubstanceService.createSubstance(vm.newSubstance).then(
                        function (data) {
                            vm.newSubstance = data;
                                ...
                                function (atts) {
                                    saveAttributes().then(
                                        function (attributes) {
                                            ...
                                            vm.attributes = [];
                                            vm.customAttributes = [];
                                            $scope.callback(vm.newSubstance);
                                            vm.newSubstance = {
                                                id: null,
                                                type: null,
                                                number: null,
                                            ...
                                            $rootScope.hideBusyIndicator();
                                            $rootScope.showSuccessMessage(subCreatedMsg);
            ...
            function createSubstanceObject() {
                create().then(
                    function () {
                        vm.newSubstance = {
                            id: null,
            ...
            function validate() {
                var valid = true;
                if (vm.newSubstance.type == null || vm.newSubstance.type == undefined ||
                    vm.newSubstance.type == "") {
                    $rootScope.showErrorMessage(subTypeValidation);
                    valid = false;
                }
                else if (vm.newSubstance.name == null || vm.newSubstance.name == undefined ||
                    vm.newSubstance.name == "") {
                    $rootScope.showErrorMessage(subNameValidation);
                    valid = false;
                }
                else if (vm.newSubstance.number == null || vm.newSubstance.number == undefined ||
                    vm.newSubstance.number == "") {
                    $rootScope.showErrorMessage(subNumberValidation);
                    valid = false;
                }
                else if (vm.newSubstance.casNumber == null || vm.newSubstance.casNumber == undefined ||
                    vm.newSubstance.casNumber == "") {
                    $rootScope.showErrorMessage(casNumberValidation);
                    valid = false;
                }
            ...
            function saveAttributes() {
                ...
                if (vm.imageAttributes.length > 0 || vm.attachmentAttributes.length > 0) {
                    angular.forEach(vm.imageAttributes, function (imgAtt) {
                            ClassificationService.updateAttributeImageValue("PGCOBJECTTYPE", vm.newSubstance.id, imgAtt.id.attributeDef, vm.images.get(imgAtt.id.attributeDef)).then(
                                function (data) {
                                    defered.resolve();
                                }
                                , function (error) {
                    ...
                    angular.forEach(vm.attachmentAttributes, function (imgAtt) {
                            ClassificationService.updateAttributeAttachmentValues("PGCOBJECTTYPE", vm.newSubstance.id, imgAtt.id.attributeDef, vm.attachments.get(imgAtt.id.attributeDef)).then(
                                function (data) {
                                    defered.resolve();
                                }
                                , function (error) {
            ...
            function saveCustomAttributes() {
                ...
                if (vm.customImageAttributes.length > 0 || vm.customAttachmentAttributes.length > 0) {
                    angular.forEach(vm.customImageAttributes, function (imgAtt) {
                            ClassificationService.updateAttributeImageValue("PGCOBJECT", vm.newSubstance.id, imgAtt.id.attributeDef, vm.customImages.get(imgAtt.id.attributeDef)).then(
                                function (data) {
                                    defered.resolve();
                                }, function (error) {
                                    $rootScope.showErrorMessage(error.message);
                    ...
                    angular.forEach(vm.customAttachmentAttributes, function (imgAtt) {
                            ClassificationService.updateAttributeAttachmentValues("PGCOBJECT", vm.newSubstance.id, imgAtt.id.attributeDef, vm.customAttachments.get(imgAtt.id.attributeDef)).then(
                                function (data) {
                                    defered.resolve();
                                }, function (error) {
                                    $rootScope.showErrorMessage(error.message);
            ...
            (function () {
                loadCustomAttributeProperties();
                $rootScope.$on('app.substance.new', createSubstanceObject);
            })();
        }
    }
)
