define(
    ...
    function (module) {
        ...
        function FileReplaceController($scope, $rootScope, $timeout, $state, $stateParams, $cookies, $translate,
                                       CommonService, SpecificationsService, DialogService) {
            var vm = this;
            ...
            vm.redId = $scope.data.reqId;
            vm.mode = $scope.data.reqMode;

            vm.spec = $stateParams.specId;
            if (vm.spec != null && vm.mode != "REQUIREMENT") {
                vm.specId = vm.spec;
            }
            if (vm.redId != null && vm.mode == "REQUIREMENT") {
                vm.specId = vm.redId;
            }
            ...
            function initFilesTableDropzone() {
                ...
                var dropZone = new Dropzone(document.querySelector('#specReplaceFiles'), { // Make the whole body a dropzone
                    url: "api/rm/specifications/" + vm.specId + "/files/replaceFile/" + vm.file.id, // Set the url
                    thumbnailWidth: 80,
                    thumbnailHeight: 80,
                    timeout: 500000,
                ...
                dropZone.on("queuecomplete", function (progress) {
                    ...
                    vm.fileConfig.fileSizeError = false;
                    $rootScope.hideSidePanel();
                    $scope.callback(vm.fileConfig.error);
                });
