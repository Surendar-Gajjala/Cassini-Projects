define(
    ...
    function (module) {
        module.controller('RequirementsFilesController', RequirementsFilesController);

        function RequirementsFilesController($scope, $rootScope, $timeout, $state, $stateParams, $cookies, $translate,
            ...
            vm.requirementId = $stateParams.requirementId;
            vm.files = [];
            ...
            vm.selectReqFile = selectReqFile;
            ...
            function loadFiles() {
                SpecificationsService.getRmObjectFiles(reqId).then(
                    function (data) {
                        vm.files = data;
            ...
            function initFilesTableDropzone(previewTemplate) {
                ...
                var myDropzone = new Dropzone(document.querySelector("#selectReqFiles"), { // Make the whole body a dropzone
                    url: "api/rm/specifications/" + reqId + "/files", // Set the url
                    thumbnailHeight: 80,
                    thumbnailWidth: 80,
                    parallelUploads: 1,
                    timeout: 500000,
            ...
            function selectReqFile() {
                $('#selectReqFiles')[0].click();
            }
            ...
            function downloadFile(file) {
                ...
                SpecificationsService.updateFileDownloadHistory(reqId, file.id).then(
                    function (data) {
            ...
            function filePreview(file) {
                ...
                var url = "{0}//{1}/api/rm/specifications/{2}/files/{3}/preview".
                    format(window.location.protocol, window.location.host, reqId, fileId);
            ...
            function lockFile(file, lock) {
                ...
                SpecificationsService.updateSpecFile(reqId, file).then(
                    function (data) {
                        file = data;
            ...
            var fileDeletedMessage = parsed.html($translate.instant("REQ_FILE_DELETED_MESSAGE")).html();
            ...
            function deleteFile(file) {
                var options = {
                    ...
                    message: $scope.deleteFileMessage + "[ " + file.name + " ] " + itemDelete + "?",
                    okButtonClass: 'btn-danger'
                };
                DialogService.confirm(options, function (yes) {
                    if (yes == true) {
                        $rootScope.showBusyIndicator();
                        SpecificationsService.deleteSpecFile(reqId, file.id).then(
                            function (data) {
            ...
            function renameFile(fileRowObject) {
                ...
                swal({
                    ...
                    if (!vm.itemRename) {
                        SpecificationsService.updateSpecFileName(reqId, fileId, newFileName).then(
                            function (data) {
            ...
            function fileReplace(file) {
                var options = {
                    ...
                    callback: function () {
                        SpecificationsService.getLatestUploadedObjectFile(reqId, file.id).then(
                            function (data) {
                                file.id = data.id;
                                file.name = data.name;
            ...
            var fileMovedToItem = parsed.html($translate.instant("FILE_MOVED_TO_REQ")).html();
            ...
            function updateFile(fileObject, folder, parent) {
                ...
                SpecificationsService.moveSpecFileToFolder(reqId, fileObject).then(
                    function (data) {
            ...
            function toggleNode(folder) {
                ...
                if (folder.expanded == false) {
                    ...
                    SpecificationsService.getSpecFolderChildren(reqId, folder.id).then(
                        function (data) {
            ...
            function deleteFolder(folder) {
                ...
                DialogService.confirm(options, function (yes) {
                    if (yes == true) {
                        $rootScope.showBusyIndicator($('.view-content'));
                        SpecificationsService.deleteSpecFolder(reqId, folder.id).then(
                            function (data) {
                                if (folder.folderChildren != undefined) {
            ...
            $rootScope.addReqFolder = addReqFolder;
            function addReqFolder(folder) {
                var options = {
                    title: newFolder,
            ...
            function loadFolderFileId(folder) {
                document.getElementById(vm.folderId).onchange = function () {
                    ...
                    $rootScope.showBusyIndicator($(".view-content"));
                    SpecificationsService.uploadSpecFolderFiles(reqId, vm.folderId, vm.importFiles).then(
                        function (data) {
                            removeChildren(folder);
                            folder.expanded = false;
                            $timeout(function () {
            ...
            function selectClipboardFile(file) {
                ...
                if (vm.selectedFiles.length > 0) {
                    $rootScope.showCopyReqFilesToClipBoard = true;
                } else {
                    $rootScope.showCopyReqFilesToClipBoard = false;
                }
            }

            $rootScope.copyReqFilesToClipBoard = copyReqFilesToClipBoard;
            function copyReqFilesToClipBoard() {
            ...
            $rootScope.clearAndCopyReqFilesToClipBoard = clearAndCopyReqFilesToClipBoard;
            function clearAndCopyReqFilesToClipBoard() {
                $application.clipboard.files = [];
                copyReqFilesToClipBoard();
            }
            ...
            function pasteFilesFromClipboard(folder, type) {
                ...
                SpecificationsService.pasteObjectFilesFromClipboard(reqId, fileId, $application.clipboard.files).then(
                    function (data) {
                        vm.copiedData = data;
                        ...
                        if ((clipboardFilesCount - vm.copiedData.length) > 0) {
                            if (vm.copiedData.length == 0) {
                                ...
                                $rootScope.showSuccessMessage("[ " + vm.copiedData.length + " ] " + filesCopiedMessage + " " + and + " [ " + (clipboardFilesCount - vm.copiedData.length) + " ] File(s) already exist", true, "REQUIREMENT");
                            }
                        } else {
                            $rootScope.showSuccessMessage("[ " + vm.copiedData.length + " ] " + filesCopiedMessage, true, "REQUIREMENT");
                        }
                        $rootScope.hideBusyIndicator();
                    }, function (error) {
                        $rootScope.showErrorMessage(error.message);
            ...
            $rootScope.undoCopiedReqFiles = undoCopiedReqFiles;
            function undoCopiedReqFiles() {
                if (vm.copiedData.length > 0) {
                    $rootScope.closeNotification();
                    $rootScope.showBusyIndicator($(".view-content"));
                    SpecificationsService.undoCopiedObjectFiles(reqId, vm.copiedData).then(
                        function (data) {
                            if (vm.copiedData.length > 0) {
            ...
            function loadReqFilesByFileName(name) {
                SpecificationsService.getSpecFilesByName(reqId, name).then(
                    function (data) {
                        vm.files = data;
                        vm.loading = false;
            ...
            (function () {
                $scope.$on('app.requirement.tabActivated', function (event, data) {
                    ...
                    $scope.$on('app.requirement.loadFiles', function (event, data) {
                        loadReqFilesByFileName(data.name);
                    })
                    loadFiles();
                });
            })();
