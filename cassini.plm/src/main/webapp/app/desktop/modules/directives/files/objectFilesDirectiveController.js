define(
    ...
    function (module) {
        module.directive('objectFilesView', ObjectFilesViewController);

        function ObjectFilesViewController($rootScope, $injector, $sce, $timeout, $translate, $application,
                                           ObjectFileService, ItemService, ItemFileService, ForgeService, CommonService,
                                           DialogService, DocumentService) {
            return {
                templateUrl: 'app/desktop/modules/directives/files/objectFilesView.jsp',
                restrict: 'E',
                scope: {
                    'objectType': '@',
                    'objectId': '=',
                ...
                link: function ($scope, $elem, attrs) {
                    ...
                    $scope.shareAllFileAndFolders = shareAllFileAndFolders;
                    ...
                    var fileDeleteMessage = parsed.html($translate.instant("FILE_DELETE_MESSAGE")).html();
                    ...
                    var fileMovedToItem = parsed.html($translate.instant("FILE_MOVED_TO_QUALITY_TYPE")).html();
                    ...
                    function sortByName(allFiles) {
                        ...
                        angular.forEach(allFiles, function (file) {
                            if (file.fileType == "FOLDER" && file.parentFile == null) {
                                folders.push(file);
                            } else if (file.fileType == "FILE" && file.parentFile == null) {
                                files.push(file);
                            }
                        });
                        folders.sort(function (a, b) {
                            ...
                            for (var i = 0; i < a.name.length; i++) {
                                var _a = a.name.charAt(i),
                                    _b = b.name.charAt(i),
                                    _c = _a.localeCompare(_b);

                                if (_c != 0) {
                                    var result = isNaN(_a) || isNaN(_b) ? a.name.localeCompare(b.name) : _a - _b;
                                    console.log(result);
                                    return result;
                                }
                            }
                        ...
                        files.sort(function (a, b) {
                            ...
                            for (var i = 0; i < a.name.length; i++) {
                                ...
                                if (_c != 0) return isNaN(_a) || isNaN(_b) ? a.name.localeCompare(b.name) : _a - _b;
                            }
                        });
                        $scope.files = [];
                        $scope.files = folders;
                    ...
                    function visitChildren(parent, name) {
                        angular.forEach(parent.children, function (file) {
                            file.searchExist = false;
                            if (file.name.toLowerCase().includes(name.toLowerCase())) {
                                file.searchExist = true;
                            }
                            ...
                            file = visitChildren(file, name);
                        });

                        angular.forEach(parent.children, function (file) {
                            if (file.searchExist) {
                                parent.searchExist = true;
                            }
                        })
                        return parent;
                    }
                    ...
                    function visitFileSearchChildren(parent, lastIndex) {
                        angular.forEach(parent.children, function (obj) {
                            if (obj.searchExist) {
                                lastIndex++;
                                obj.level = parent.level + 1;
                                obj.expanded = false;
                                obj.folderChildren = [];
                                obj.parentFolder = parent;
                                if ($scope.objectType == "DOCUMENT" || $scope.objectType == "MFRPARTINSPECTIONREPORT" || $scope.objectType == "PPAPCHECKLIST") {
                                    angular.forEach(obj.reviewers, function (reviewer) {
                                        if (reviewer.approver && $scope.personDetails.id == reviewer.reviewer && reviewer.status == 'NONE') {
                                            obj.approver = true;
                                ...
                                $scope.files.splice(lastIndex, 0, obj);
                                parent.folderChildren.push(obj);
                                parent.expanded = true;
                                lastIndex = visitFileSearchChildren(obj, lastIndex)
                            }
                        });
                    ...
                    function loadHasPermission() {
                        ...
                        if ($scope.objectType == 'DOCUMENT') {
                            ...
                            $scope.hasSignOff = $rootScope.hasPermission(type, 'signoff');
                        }
                        if ($rootScope.hasPermission(type, 'teamCreate')) {
                            $scope.hasCreate = true;
                        }
                        if ($rootScope.hasPermission(type, 'teamDownload')) {
                            $scope.hasDownload = true;
                        }
                    }
                    ...
                    function handleDragEnter(e) {
                        $("#qualityFilesTableContainer")[0].classList.add('drag-over');
                        //$scope.showFileDropzone = true;

                    }

                    function handleDragLeave(e) {
                        $("#qualityFilesTableContainer")[0].classList.remove('drag-over');
                        //$scope.showFileDropzone = false;
                    }
                    ...
                    function loadObjectDetailCounts() {
                        ...
                        } else if ($scope.objectType == "BOPROUTEOPERATION") {
                    ...
                    function initFilesTableDropzone(previewTemplate) {
                        ...
                        if (Dropzone.instances.length > 0) {
                            if ($scope.objectType == "DOCUMENT") {
                                angular.forEach(Dropzone.instances, function (instance) {
                                    instance.options.url = "api/plm/objects/" + $scope.objectId + "/" + $scope.objectType + "/files/" + $scope.objectId;
                                })
                            } else if ($scope.objectType == "MANUFACTURERPART" || $scope.objectType == "MFRPARTINSPECTIONREPORT") {
                                angular.forEach(Dropzone.instances, function (instance) {
                                    instance.options.url = "api/plm/objects/" + $scope.objectId + "/" + $scope.objectType + "/files/" + 0;
                                })
                            }
                        }
                        var dropZone = new Dropzone(document.querySelector('#qualityObjectFiles'), { // Make the whole body a dropzone
                        ...
                        dropZone.on("queuecomplete", function (progress) {
                            $("#total-progress").hide();
                            $("#qualityFilesTableContainer").removeClass('drag-over');
                            $scope.showFileDropzone = false;
                        ...
                        $("#qualityFilesTableContainer").on('dragover', handleDragEnter);
                        $("#qualityFilesTableContainer").on('dragleave', handleDragLeave);
                        $("#qualityFilesTableContainer").on('drop', handleDragLeave);
                                ...
                                if ($scope.selectedFiles.length > 0) {
                                    angular.forEach($scope.selectedFiles, function (selectedFile) {
                                        if (selectedFile.id == item.id) {
                                            $scope.selectedFiles.splice($scope.selectedFiles.indexOf(selectedFile), 1);
                                            selectedFilesMap.remove(selectedFile.id);
                                        }
                                    })
                                }
                                removeChildren(item);
                    ...
                    function selectFolderFiles(file) {
                        angular.forEach($scope.files, function (fileData) {
                            if (fileData.parentFile == file.id) {
                                fileData.selected = true;
                                if (selectedFilesMap.get(fileData.id) == null) {
                                    $scope.selectedFiles.push(fileData);
                                    selectedFilesMap.put(fileData.id, fileData);
                                }
                                if (fileData.fileType == "FOLDER") {
                                    selectFolderFiles(fileData);
                                }
                            }
                        })
                    }

                    function selectParentFolder(parentFolder) {
                        var count = 0;
                        angular.forEach(parentFolder.folderChildren, function (child) {
                            if (child.selected) {
                                count++;
                            }
                            if (count == parentFolder.folderChildren.length) {
                                parentFolder.selected = true;
                                if (selectedFilesMap.get(parentFolder.id) == null) {
                                    $scope.selectedFiles.push(parentFolder);
                                    selectedFilesMap.put(parentFolder.id, parentFolder);
                                }
                                if (parentFolder.parentFolder != null && parentFolder.parentFolder != undefined) {
                                    selectParentFolder(parentFolder.parentFolder);
                                }
                            }
                        })
                    }

                    function unSelectParentFolder(parentFolder) {
                        angular.forEach($scope.files, function (fileData) {
                            if (fileData.id == parentFolder.id) {
                                fileData.selected = false;
                                angular.forEach($scope.selectedFiles, function (selectedFile) {
                                    if (selectedFile.id == parentFolder.id) {
                                        $scope.selectedFiles.splice($scope.selectedFiles.indexOf(selectedFile), 1);
                                        selectedFilesMap.remove(selectedFile.id);
                                    }
                                })
                                if (parentFolder.parentFolder != null && parentFolder.parentFolder != undefined) {
                                    unSelectParentFolder(parentFolder.parentFolder);
                                }
                            }
                        })
                    }

                    function unSelectFolderFiles(file) {
                        angular.forEach($scope.files, function (fileData) {
                            if (fileData.parentFile == file.id) {
                                fileData.selected = false;
                                angular.forEach($scope.selectedFiles, function (selectedFile) {
                                    if (selectedFile.parentFile == file.id) {
                                        $scope.selectedFiles.splice($scope.selectedFiles.indexOf(selectedFile), 1);
                                        selectedFilesMap.remove(selectedFile.id);
                                    }
                                })
                                if (fileData.fileType == "FOLDER") {
                                    unSelectFolderFiles(fileData);
                                }
                            }
                        })
                    }

                    var selectedFilesMap = new Hashtable();
                    $scope.selectedFiles = [];
                    $scope.selectFile = selectFile;
                    function selectFile(file) {
                        if (file.selected) {
                            if ((file.locked && file.lockedBy == $scope.personDetails.id) || !file.locked) {
                                $scope.selectedFiles.push(file);
                                selectedFilesMap.put(file.id, file);
                                if (file.fileType == "FOLDER") {
                                    ...
                                    if (file.parentFolder != null && file.parentFolder != undefined) {
                                        selectParentFolder(file.parentFolder);
                                    }
                                } else {
                                    if (file.parentFolder != null && file.parentFolder != undefined && file.parentFolder.folderChildren != null && file.parentFolder.folderChildren != undefined && !file.parentFolder.selected) {
                                        var count = 0;
                                        angular.forEach(file.parentFolder.folderChildren, function (child) {
                                            if (child.selected) {
                                                count++;
                                            }
                                            if (count == file.parentFolder.folderChildren.length) {
                                                file.parentFolder.selected = true;
                                                if (selectedFilesMap.get(file.parentFolder.id) == null) {
                                                    $scope.selectedFiles.push(file.parentFolder);
                                                    selectedFilesMap.put(file.parentFolder.id, file.parentFolder);
                                                }
                                                if (file.parentFolder.parentFolder != null && file.parentFolder.parentFolder != undefined) {
                                                    selectParentFolder(file.parentFolder.parentFolder);
                                                }
                                            }
                                        })
                                    }
                            ...
                            if ($scope.selectedFiles.length == $scope.files.length) {
                                $scope.object.checked = true;
                            } else {
                                $scope.object.checked = false;
                            }
                        } else {
                            if (file.fileType == "FOLDER") {
                                angular.forEach($scope.files, function (fileData) {
                                ...
                                if (file.parentFolder != null && file.parentFolder != undefined) {
                                    unSelectParentFolder(file.parentFolder);
                                }
                            } else {
                                if (file.parentFolder != null && file.parentFolder != undefined) {
                                    angular.forEach($scope.files, function (fileData) {
                                        if (fileData.id == file.parentFolder.id) {
                                            fileData.selected = false;
                                            angular.forEach($scope.selectedFiles, function (selectedFile) {
                                                if (selectedFile.id == file.parentFolder.id) {
                                                    $scope.selectedFiles.splice($scope.selectedFiles.indexOf(selectedFile), 1);
                                                    selectedFilesMap.remove(selectedFile.id);
                                                }
                                            })
                                            if (file.parentFolder.parentFolder != null && file.parentFolder.parentFolder != undefined) {
                                                unSelectParentFolder(file.parentFolder.parentFolder);
                                            }
                                        }
                                    })
                                }
                            ...
                            selectedFilesMap.remove(file.id);
                        }

                        if ($scope.selectedFiles.length > 0) {
                            $rootScope.showShareAllFileAndFoldersToUsers = true;
                            $rootScope.showCopyObjectFilesToClipBoard = true;
                        } else {
                            $scope.object.checked = false;
                            $rootScope.showShareAllFileAndFoldersToUsers = false;
                    ...
                    /*
                     * Share File or Folders
                     * */

                    $scope.selectedSharedFoldersAndFiles = [];
                    $scope.object = {checked: false};
                    $rootScope.showShareAllFileAndFoldersToUsers = false;
                    function shareAllFileAndFolders() {
                        if ($scope.object.checked) {
                            $scope.object.checked = true;
                            $scope.selectedFiles = [];
                            selectedFilesMap = new Hashtable();
                            angular.forEach($scope.files, function (file) {
                                file.selected = true;
                                $scope.selectedFiles.push(file);
                                selectedFilesMap.put(file.id, file);
                    ...
                    $rootScope.shareAllFilesAndFoldersToUsers = shareAllFilesAndFoldersToUsers;
                    function shareAllFilesAndFoldersToUsers(type) {
                    ...
                    function shareFilesFolders(type) {
                        var options = {
                            title: "Share files or folders",
                            template: 'app/desktop/modules/shared/share/shareFilesFoldersView.jsp',
                            controller: 'ShareFilesFoldersController as shareFilesFoldersVm',
                            resolve: 'app/desktop/modules/shared/share/shareFilesFolderController',
                            width: 600,
                            showMask: true,
                            data: {
                                objectId: $scope.objectId,
                                sharedFiles: $scope.selectedFiles,
                                objectType: $scope.objectType,
                                fileShareType: type
                            },
                            buttons: [
                                {text: shareButton, broadcast: 'app.shareSelected.files.folders'}
                            ],
                            callback: function (data) {
                                $scope.object.checked = false;
                                angular.forEach($scope.files, function (file) {
                    ...
                    $rootScope.copyObjectFilesToClipBoard = copyObjectFilesToClipBoard;
                    function copyObjectFilesToClipBoard() {
                    ...
                    $rootScope.clearAndCopyObjectFilesToClipBoard = clearAndCopyObjectFilesToClipBoard;
                    function clearAndCopyObjectFilesToClipBoard() {
                        $application.clipboard.files = [];
                        $scope.clipboardFiles = [];
                        copyObjectFilesToClipBoard();
                    }
                    ...
                    function pasteFilesFromClipboard(folder, type) {
                        ...
                        angular.forEach($application.clipboard.files, function (file) {
                            file.parentFolder = null;
                            file.folderChildren = [];
                        });
                    ...
                    function showDocumentSubmitDialog(type, file) {
                        ...
                        $timeout(function () {
                            $('#' + $scope.objectType + ' #document-modal').css("display", "block");
                        }, 500);
                    }
                    ...
                    function hideDocumentSubmitDialog() {
                        ...
                        //modal.style.display = "none";
                        $('#' + $scope.objectType + ' #document-modal').css("display", "none");
                    ...
                    $scope.getClass = function (file) {
                        if ($(window).height() - $('.row-' + file.id).position().top < dropdownHeight) {
                            return 'dropup';
                        }
                        return 'dropdown';
                    };
                    ...
                    (function () {
                        $scope.$on('app.objectFile.tabActivated', function (event, data) {
