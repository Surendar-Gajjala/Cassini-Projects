define(
    ...
    function (module) {
        module.directive('objectAttributeDetailsView', ObjectAttributeDetailsViewDirective);

        function ObjectAttributeDetailsViewDirective($rootScope, $compile, $sce, $timeout, $state, $translate, $application, DialogService, ECOService, ObjectAttributeService,
                                                     AttributeAttachmentService, ObjectTypeAttributeService, ItemService, WorkflowDefinitionService, MfrService, $window, QualityTypeService,
                                                     CommonService, MfrPartsService, SpecificationsService, InspectionPlanService, InspectionService, ProblemReportService, RequirementService, ReqDocumentService,
                                                     NcrService, QcrService, PpapService, SupplierAuditService, ProjectService, PMObjectTypeService, PGCObjectTypeService, CustomObjectService, CustomObjectTypeService, MESObjectTypeService,
                                                     SupplierService, ClassificationService, WorkflowService) {
            return {
                templateUrl: 'app/desktop/modules/directives/objectAttributeDetailsViewDirective.jsp',
                restrict: 'E',
                scope: {
                    'objectType': '@',
                    'actualObjectType': '@',
                ...
                link: function ($scope, $elem, attrs) {
                    ...
                    var objectType = null;
                    if ($scope.objectType == "PRODUCTINSPECTIONPLANTYPE") {
                        objectType = "PRODUCTINSPECTIONPLAN";
                    } else if ($scope.objectType == "MATERIALINSPECTIONPLANTYPE") {
                        objectType = "MATERIALINSPECTIONPLAN";
                    } else if ($scope.objectType == "INSPECTIONPLANREVISIONTYPE") {
                        objectType = "INSPECTIONPLANREVISION";
                    } else if ($scope.objectType == "PRTYPE") {
                        objectType = "PROBLEMREPORT";
                    } else if ($scope.objectType == "NCRTYPE") {
                        objectType = "NCR";
                    } else if ($scope.objectType == "QCRTYPE") {
                        objectType = "QCR";
                    } else if ($scope.objectType == "PPAPTYPE") {
                        objectType = "PPAP";
                    } else if ($scope.objectType == "PMOBJECTTYPE") {
                        objectType = $scope.actualObjectType;
                    } else if ($scope.objectType == "SUPPLIERAUDITTYPE") {
                        objectType = "SUPPLIERAUDIT";
                    } else if ($scope.objectType == "CHANGETYPE") {
                        objectType = "CHANGE";
                    } else if ($scope.objectType == "ITEMTYPE") {
                        objectType = "ITEM";
                    } else if ($scope.objectType == "WORKFLOWTYPE") {
                        objectType = "PLMWORKFLOW";
                    } else if ($scope.objectType == "PLMWORKFLOWSTATUS") {
                        objectType = "PLMWORKFLOWSTATUS";
                    } else if ($scope.objectType == "MANUFACTURERTYPE") {
                        objectType = "MANUFACTURER";
                    } else if ($scope.objectType == "MANUFACTURERPARTTYPE") {
                        objectType = "MANUFACTURERPART";
                    } else if ($scope.objectType == "SPECIFICATIONTYPE") {
                        objectType = "SPECIFICATION";
                    } else if ($scope.objectType == "REQUIREMENTTYPE") {
                        objectType = "REQUIREMENT";
                    } else if ($scope.objectType == "MESOBJECTTYPE") {
                        objectType = $scope.actualObjectType;
                    } else if ($scope.objectType == "MROOBJECTTYPE") {
                        objectType = $scope.actualObjectType;
                    } else if ($scope.objectType == "PGCOBJECTTYPE") {
                        objectType = $scope.actualObjectType;
                    } else if ($scope.objectType == "REQUIREMENTDOCUMENTTYPE") {
                        objectType = $scope.actualObjectType;
                    }
                    else if ($scope.objectType == "SUPPLIERTYPE") {
                        objectType = "MFRSUPPLIER";
                    }
                    else if ($scope.objectType == "CUSTOMOBJECTTYPE") {
                    ...
                    $scope.saveAttributeTitle = parsed.html($translate.instant("SAVE")).html();
                    $scope.cancelChangesTitle = parsed.html($translate.instant("CANCEL")).html();
                    ...
                    function validAttribute(attribute) {
                        ...
                        if (attribute.attributeDef.dataType == "TEXT" && attribute.attributeDef.newValidations != null && attribute.attributeDef.newValidations != "") {
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                ...
                                if (valid) {
                                    if (validation.key == "MIN_LENGTH_OF_CHARACTERS" && validation.value != null && (attribute.value.stringValue == null || attribute.value.stringValue == "")) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Enter at least " + validation.value + " characters");
                                    } else if (validation.key == "MIN_LENGTH_OF_CHARACTERS" && validation.value != null && attribute.value.stringValue.length < validation.value) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Enter at least " + validation.value + " characters");
                                    } else if (validation.key == "MAX_LENGTH_OF_CHARACTERS" && validation.value != null && attribute.value.stringValue.length > validation.value) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Count should not exceed " + validation.value + " characters");
                                    } else if (validation.key == "PATTERN" && validation.value != null && !validation.patternValue.test(attribute.value.stringValue)) {
                                        valid = false;
                                        $rootScope.showWarningMessage(attribute.attributeDef.name + " : Value should match to given pattern");
                                    }
                                }
                            })
                        } else if (attribute.attributeDef.dataType == "INTEGER" && attribute.attributeDef.newValidations != null && attribute.attributeDef.newValidations != "") {
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                if (valid) {
                                    if (validation.key == "ONLY_POSITIVE_VALUES" && validation.value && attribute.value.integerValue != null && attribute.value.integerValue < 0) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Please enter positive value");
                                    } else if (validation.key == "ONLY_NEGATIVE_VALUES" && validation.value && attribute.value.integerValue != null && attribute.value.integerValue >= 0) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Please enter negative value");
                                    } else if (validation.key == "MIN_VALUE" && validation.value != null && attribute.value.integerValue != null && attribute.value.integerValue != "" && parseInt(validation.value) > parseInt(attribute.value.integerValue)) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Value should be greater than or equal to " + validation.value);
                                    } else if (validation.key == "MAX_VALUE" && validation.value != null && attribute.value.integerValue != null && attribute.value.integerValue != "" && parseInt(validation.value) < parseInt(attribute.value.integerValue)) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Value should be less than or equal to " + validation.value);
                                    }
                                }
                            })
                        } else if (attribute.attributeDef.dataType == "DOUBLE" && attribute.attributeDef.newValidations != null && attribute.attributeDef.newValidations != "") {
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                if (valid) {
                                    if (validation.key == "ONLY_POSITIVE_VALUES" && validation.value && attribute.value.doubleValue != null && attribute.value.doubleValue < 0) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Please enter positive value");
                                    } else if (validation.key == "ONLY_NEGATIVE_VALUES" && validation.value && attribute.value.doubleValue != null && attribute.value.doubleValue >= 0) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Please enter negative value");
                                    } else if (validation.key == "MIN_VALUE" && validation.value != null && attribute.value.doubleValue != null && attribute.value.doubleValue != "" && parseInt(validation.value) > parseInt(attribute.value.doubleValue)) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Value should be greater than or equal to" + validation.value);
                                    } else if (validation.key == "MAX_VALUE" && validation.value != null && attribute.value.doubleValue != null && attribute.value.doubleValue != "" && parseInt(validation.value) < parseInt(attribute.value.doubleValue)) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Value should be less than or equal to " + validation.value);
                                    } else if (validation.key == "NO_OF_DECIMALS_TO_ENTER" && validation.value != null && validation.value != "") {
                                        var decimalCheck = attribute.value.doubleValue.toString().split('.');
                                        if (decimalCheck[1] != undefined && decimalCheck[1].length > validation.value) {
                                            valid = false;
                                            $rootScope.showWarningMessage("Decimal value should not exceed " + validation.value);
                                        }
                                    }
                                }
                            })
                        } else if (attribute.attributeDef.dataType == "CURRENCY" && attribute.attributeDef.newValidations != null && attribute.attributeDef.newValidations != "") {
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                if (valid) {
                                    if (validation.key == "ONLY_POSITIVE_VALUES" && validation.value && attribute.value.currencyValue != null && attribute.value.currencyValue < 0) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Please enter positive value");
                                    } else if (validation.key == "ONLY_NEGATIVE_VALUES" && validation.value && attribute.value.currencyValue != null && attribute.value.currencyValue >= 0) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Please enter negative value");
                                    } else if (validation.key == "NO_OF_DECIMALS_TO_ENTER" && validation.value != null && validation.value != "") {
                                        var decimalCheck = attribute.value.currencyValue.toString().split('.');
                                        if (decimalCheck[1] != undefined && decimalCheck[1].length > validation.value) {
                                            valid = false;
                                            $rootScope.showWarningMessage("Decimal value should not exceed " + validation.value);
                                        }
                            ...
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                if (valid) {
                                    if (validation.key == "MAX_LENGTH_OF_CHARACTERS" && validation.value != null && attribute.value.longTextValue.length > validation.value) {
                                        valid = false;
                                        $rootScope.showWarningMessage("Value should not exceed " + validation.value + " characters");
                                    }
                                }
                            })
                        } else if (attribute.attributeDef.dataType == "DATE" && attribute.attributeDef.newValidations != null && attribute.attributeDef.newValidations != "") {
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                if (valid) {
                                    if (validation.key == "FROM_DATE" && validation.value != null) {
                                        var fromDate = moment(attribute.value.dateValue, $rootScope.applicationDateSelectFormat);
                                        ...
                                        if (!val) {
                                            valid = false;
                                            $rootScope.showWarningMessage("Date should be on (or) after " + validation.value);
                                        }
                                    } else if (validation.key == "TO_DATE" && validation.value != null) {
                                        var toDate = moment(attribute.value.dateValue, $rootScope.applicationDateSelectFormat);
                                        var validationDate = moment(validation.value, $rootScope.applicationDateSelectFormat);
                                        var val = toDate.isSame(validationDate) || toDate.isBefore(validationDate);
                                        if (!val) {
                                            valid = false;
                                            $rootScope.showWarningMessage("Date should be on (or) before " + validation.value);
                                        }
                                    }
                                }
                            })
                            ...
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                if (valid) {
                                    if (validation.key == "FROM_TIME" && validation.value != null) {
                                        ...
                                        if (!val) {
                                            valid = false;
                                            $rootScope.showWarningMessage("Time should be on (or) after " + validation.value);
                                        }
                                    } else if (validation.key == "TO_TIME" && validation.value != null) {
                                        var toTime = moment(attribute.timeValue, 'hh:mm:ss');
                                        var validationTime = moment(validation.value, 'hh:mm:ss');
                                        var val = toTime.isSame(validationTime) || toTime.isBefore(validationTime);
                                        if (!val) {
                                            valid = false;
                                            $rootScope.showWarningMessage("Time should be on (or) before " + validation.value);
                                        }
                                    }
                                }
                            })
                        ...
                        if (attribute.attributeDef.dataType == "TEXT") {
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                if (validation.key == "ALL_CAPITAL" && validation.value) {
                                    attribute.value.stringValue = attribute.value.stringValue.toUpperCase();
                                } else if (validation.key == "ALL_SMALL" && validation.value) {
                                    attribute.value.stringValue = attribute.value.stringValue.toLowerCase();
                                }
                            })
                        } else if (attribute.attributeDef.dataType == "LONGTEXT") {
                            angular.forEach(attribute.attributeDef.newValidations, function (validation) {
                                if (validation.key == "ALL_CAPITAL" && validation.value) {
                                    attribute.value.longTextValue = attribute.value.longTextValue.toUpperCase();
                                } else if (validation.key == "ALL_SMALL" && validation.value) {
                                    attribute.value.longTextValue = attribute.value.longTextValue.toLowerCase();
                                }
                            })
                        }
                    }
                    ...
                    function saveObjectProperties(attribute) {
                        if (validAttribute(attribute) && validateAttributeValues(attribute)) {
                            var promise = null;

                            if ($scope.objectType == "PRODUCTINSPECTIONPLANTYPE" || $scope.objectType == "MATERIALINSPECTIONPLANTYPE") {
                                if (!attribute.attributeDef.revisionSpecific) {
                                    promise = InspectionPlanService.updatePlanAttribute($scope.objectId, attribute.value);
                                } else {
                                    promise = InspectionPlanService.updatePlanRevisionAttribute($scope.objectId, attribute.value);
                                }
                            } else if ($scope.objectType == "INSPECTIONPLANREVISION") {
                                promise = InspectionService.updatePlanRevisionAttribute($scope.objectId, attribute.value);
                            } else if ($scope.objectType == "PRTYPE") {
                                promise = ProblemReportService.updatePrAttribute($scope.objectId, attribute.value);
                            } else if ($scope.objectType == "NCRTYPE") {
                                promise = NcrService.updateNcrAttribute($scope.objectId, attribute.value);
                            } else if ($scope.objectType == "QCRTYPE") {
                                promise = QcrService.updateQcrAttribute($scope.objectId, attribute.value);
                            } else if ($scope.objectType == "PPAPTYPE") {
                                promise = PpapService.updatePPAPAttribute($scope.objectId, attribute.value);
                            } else if ($scope.objectType == "PMOBJECTTYPE") {
                                promise = PMObjectTypeService.updatePmObjectAttributes($scope.objectId, attribute.value);
                            } else if ($scope.objectType == "SUPPLIERAUDITTYPE") {
                                promise = SupplierAuditService.updateSupplierAuditAttribute($scope.objectId, attribute.value);
                            } else if ($scope.objectType == "CHANGETYPE") {
                                promise = ECOService.updateEcoAttribute($scope.objectId, attribute.value)
                            } else if ($scope.objectType == "ITEMTYPE") {
                                if (!attribute.attributeDef.revisionSpecific) {
                                    promise = ItemService.updateItemAttribute($scope.objectId, attribute.value);
                                } else {
                                    promise = ItemService.updateItemRevisionAttribute($scope.objectId, attribute.value);
                                }
                            } else if ($scope.objectType == "WORKFLOWTYPE") {
                                promise = WorkflowDefinitionService.updateWorkflowAttribute($scope.objectId, attribute.value)
                            } else if ($scope.objectType == "PLMWORKFLOWSTATUS") {
                                promise = WorkflowService.updateWorkflowStatusAttribute($scope.objectId, attribute.value)
                            } else if ($scope.objectType == "MANUFACTURERTYPE") {
                                promise = MfrService.updateMfrAttribute($scope.objectId, attribute.value)
                            } else if ($scope.objectType == "MANUFACTURERPARTTYPE") {
                                promise = MfrPartsService.updateMfrPartAttribute($scope.objectId, attribute.value)
                            } else if ($scope.objectType == "SPECIFICATIONTYPE" || $scope.objectType == "REQUIREMENTTYPE") {
                                promise = SpecificationsService.updateSpecAttribute($scope.objectId, attribute.value)
                            } else if ($scope.objectType == "CUSTOMOBJECTTYPE") {
                                promise = CustomObjectService.updateCustomAttribute($scope.objectId, attribute.value)
                            } else if ($scope.objectType == "MESOBJECTTYPE") {
                                promise = MESObjectTypeService.updateMESObjectAttribute($scope.objectId, attribute.value)
                            } else if ($scope.objectType == "MROOBJECTTYPE") {
                                promise = MESObjectTypeService.updateMROObjectAttribute($scope.objectId, attribute.value)
                            }
                            else if ($scope.objectType == "PGCOBJECTTYPE") {
                                promise = PGCObjectTypeService.updatePGCObjectAttribute($scope.objectId, attribute.value)
                            }
                            else if ($scope.objectType == "REQUIREMENTDOCUMENTTYPE") {
                                promise = PMObjectTypeService.updateReqObjectAttribute($scope.objectId, attribute.value)
                            }
                            else if ($scope.objectType == "SUPPLIERTYPE") {
                                promise = SupplierService.updateSupplierAttribute($scope.objectId, attribute.value)
                            }
                            if (promise != null) {
                                promise.then(
                                    function (data) {
                    ...
                    function saveAttachments(attribute) {
                        ...
                        if (attribute.attachmentValues.length > 0) {
                            var type = objectType;
                            if (attribute.attributeDef.revisionSpecific && $scope.objectType == "ITEMTYPE") {
                                type = "ITEMREVISION";
                            } else if (attribute.attributeDef.revisionSpecific && ($scope.objectType == "PRODUCTINSPECTIONPLANTYPE" || $scope.objectType == "MATERIALINSPECTIONPLANTYPE")) {
                                type = "INSPECTIONPLANREVISION";
                            }
                                    ...
                                    if ((attribute.id.objectId == ecoAttachment.objectId) && (attribute.id.attributeDef == ecoAttachment.attributeDef) && (attachmentValue.name == ecoAttachment.name)) {
                                        ...
                                        DialogService.confirm(options, function (yes) {
                                            if (yes == true) {
                                                ...
                                                AttributeAttachmentService.saveAttributeAttachment(attribute.id.objectId, attribute.id.attributeDef, type, attachmentValue).then(
                                                    function (data) {
                    ...
                    function deleteAttachments(attribute, attachment) {
                        var options = {
                            ...
                            message: $rootScope.deleteAttachmentConfirmation.format(attachment.name),
                            okButtonClass: 'btn-danger'
                        }
                        DialogService.confirm(options, function (yes) {
                            if (yes == true) {
                    ...
                    function showRefValueDetails(attribute) {
                        ...
                        else if (attribute.value.refValue.objectType == 'PPAP') {
                            $state.go("app.pqm.ppap.details", {
                                qcrId: attribute.value.refValue.id,
                                tab: 'details.basic'
                            });
                        }
                        else if (attribute.value.refValue.objectType == 'SUPPLIERAUDIT') {
                    ...
                    function editListValue(attribute) {
                        attribute.editMode = true;
                        $scope.listValue = attribute.value.listValue;
                        $scope.mlistValue = angular.copy(attribute.value.mlistValue);
                    }

                    $scope.cancelListValue = cancelListValue;
                    function cancelListValue(attribute) {
                        attribute.value.listValue = $scope.listValue;
                        attribute.value.mlistValue = angular.copy($scope.mlistValue);
                        attribute.editMode = false;
                    }

                    $scope.validationPopover = "right";
                    function loadObjectTypeAttributes() {
                        ...
                        if ($scope.objectType == "INSPECTIONPLANREVISIONTYPE") {
                            type = "INSPECTIONPLANTYPE"
                        }
                        if ($scope.objectType != "CUSTOMOBJECTTYPE") {
                            promise = ClassificationService.getObjectTypeAttributesWithHierarchy(type, $scope.objectTypeId, $scope.objectId)
                        } else if ($scope.objectType == "CUSTOMOBJECTTYPE") {
                            promise = CustomObjectTypeService.getCustomObjectTypeAttributes(type, $scope.objectTypeId)
                        }
                        if (promise != null) {
                            promise.then(
                                function (data) {
                                    if ($scope.objectType == "CHANGETYPE") {
                                        $scope.objectTypeAttributes = data.changeTypeAttributes;
                                    } else if ($scope.objectType == "MANUFACTURERTYPE") {
                                        $scope.objectTypeAttributes = data.manufacturerTypeAttributes;
                                    } else if ($scope.objectType == "WORKFLOWTYPE") {
                                        $scope.objectTypeAttributes = data.workflowTypeAttributes;
                                    } else if ($scope.objectType == "PLMWORKFLOWSTATUS") {
                                        $scope.validationPopover = "left";
                                        $scope.objectTypeAttributes = data.workflowActivityFormFields;
                                    } else if ($scope.objectType == "MANUFACTURERPARTTYPE") {
                                        $scope.objectTypeAttributes = data.partTypeAttributes;
                                    } else if ($scope.objectType == "ITEMTYPE") {
                                        $scope.objectTypeAttributes = data.itemTypeAttributes;
                                    } else if ($scope.objectType == "SPECIFICATIONTYPE" || $scope.objectType == "REQUIREMENTTYPE") {
                                        $scope.objectTypeAttributes = data.rmObjectTypeAttributes;
                                    } else if ($scope.objectType == "CUSTOMOBJECTTYPE") {
                                        $scope.objectTypeAttributes = data.customObjectTypeAttributes;
                                    } else if ($scope.objectType == "MESOBJECTTYPE") {
                                        $scope.objectTypeAttributes = data.mesObjectTypeAttributes;
                                    } else if ($scope.objectType == "MROOBJECTTYPE") {
                                        $scope.objectTypeAttributes = data.mroObjectTypeAttributes;
                                    } else if ($scope.objectType == "PGCOBJECTTYPE") {
                                        $scope.objectTypeAttributes = data.pgcObjectTypeAttributes;
                                    } else if ($scope.objectType == "REQUIREMENTDOCUMENTTYPE") {
                                        $scope.objectTypeAttributes = data.requirementObjectTypeAttributes;
                                    }
                                    else if ($scope.objectType == "SUPPLIERTYPE") {
                                        $scope.objectTypeAttributes = data.supplierTypeAttributes;
                                    } else if ($scope.objectType == "PMOBJECTTYPE") {
                                        $scope.objectTypeAttributes = data.pmObjectTypeAttributes;
                                    }
                                    else {
                                        $scope.objectTypeAttributes = data.qualityTypeAttributes;
                                    }
                                    angular.forEach($scope.objectTypeAttributes, function (attr) {
                                        if (attr.validations != null) {
                                            attr.newValidations = JSON.parse(attr.validations);
                                        }
                                        var objectId = $scope.objectId;
                                        if (attr.revisionSpecific != null && attr.revisionSpecific != undefined && !attr.revisionSpecific) {
                                            if ($scope.masterId != null) {
                                                objectId = $scope.masterId;
                                            }
                                        }
                                        if (attr.visible) {
                                            var att = {
                                            ...
                                            if (attr.attributeGroup != null && attr.attributeGroup != "") {
                                                var groupAttribute = groupAttributeMap.get(attr.attributeGroup);
                                                if (groupAttribute == null) {
                                                    var groupAttributes = {
                                                        groupName: attr.attributeGroup,
                                                        objectProperties: [att]
                                                    }
                                                    groupAttributeMap.put(attr.attributeGroup, groupAttributes);
                                                } else {
                                                    groupAttribute.objectProperties.push(att);
                                                    groupAttributeMap.put(attr.attributeGroup, groupAttribute)
                                                }
                                            } else {
                    ...
                    function loadObjectAttributeValues() {
                        var promise = null;
                        var type = $scope.objectType;
                        if ($scope.objectType == "INSPECTIONPLANREVISIONTYPE") {
                            type = "INSPECTIONPLANTYPE"
                        ...
                        if ($scope.objectType != "CUSTOMOBJECTTYPE") {
                            promise = ClassificationService.getObjectAttributeValues(type, $scope.objectTypeId, $scope.objectId)
                        } else if ($scope.objectType == "CUSTOMOBJECTTYPE") {
                            promise = CustomObjectTypeService.getCustomObjectTypeAttributeValues(type, $scope.objectTypeId, $scope.objectId)
                        }
                        if (promise != null) {
                            promise.then(
                                function (data) {
                                    if ($scope.objectType == "PRODUCTINSPECTIONPLANTYPE" || $scope.objectType == "MATERIALINSPECTIONPLANTYPE") {
                                        $scope.attributeValues = data.inspectionPlanAttributes;
                                        if ($scope.attributeValues.length > 0) {
                                            Array.prototype.push.apply($scope.attributeValues, data.inspectionPlanRevisionAttributes);
                                        } else {
                                            $scope.attributeValues = data.inspectionPlanRevisionAttributes;
                                        }
                                    } else if ($scope.objectType == "INSPECTIONPLANREVISIONTYPE") {
                                        $scope.attributeValues = data.inspectionPlanRevisionAttributes;
                                    } else if ($scope.objectType == "PRTYPE") {
                                        $scope.attributeValues = data.problemReportAttributes;
                                    } else if ($scope.objectType == "NCRTYPE") {
                                        $scope.attributeValues = data.ncrAttributes;
                                    } else if ($scope.objectType == "QCRTYPE") {
                                        $scope.attributeValues = data.qcrAttributes;
                                    } else if ($scope.objectType == "PPAPTYPE") {
                                        $scope.attributeValues = data.ppapAttributes;
                                    } else if ($scope.objectType == "PMOBJECTTYPE") {
                                        $scope.attributeValues = data.objectAttributes;
                                    } else if ($scope.objectType == "SUPPLIERAUDITTYPE") {
                                        $scope.attributeValues = data.supplierAuditAttributes;
                                    } else if ($scope.objectType == "CHANGETYPE") {
                                        $scope.attributeValues = data.changeAttributes;
                                    } else if ($scope.objectType == "ITEMTYPE") {
                                        $scope.attributeValues = data.itemAttributes;
                                        if ($scope.attributeValues.length > 0) {
                                            Array.prototype.push.apply($scope.attributeValues, data.itemRevisionAttributes);
                                        } else {
                                            $scope.attributeValues = data.itemRevisionAttributes;
                                        }
                                    } else if ($scope.objectType == "WORKFLOWTYPE") {
                                        $scope.attributeValues = data.workflowAttributes;
                                    } else if ($scope.objectType == "PLMWORKFLOWSTATUS") {
                                        $scope.attributeValues = data.workflowActivityFormData;
                                    } else if ($scope.objectType == "MANUFACTURERTYPE") {
                                        $scope.attributeValues = data.manufacturerAttributes;
                                    } else if ($scope.objectType == "MANUFACTURERPARTTYPE") {
                                        $scope.attributeValues = data.partAttributes;
                                    } else if ($scope.objectType == "SPECIFICATIONTYPE" || $scope.objectType == "REQUIREMENTTYPE") {
                                        $scope.attributeValues = data.rmObjectAttributes;
                                    } else if ($scope.objectType == "CUSTOMOBJECTTYPE") {
                                        $scope.attributeValues = data.customObjectAttributes;
                                    } else if ($scope.objectType == "MESOBJECTTYPE") {
                                        $scope.attributeValues = data.mesObjectAttributes;
                                    } else if ($scope.objectType == "MROOBJECTTYPE") {
                                        $scope.attributeValues = data.mroObjectAttributes;
                                    } else if ($scope.objectType == "PGCOBJECTTYPE") {
                                        $scope.attributeValues = data.pgcObjectAttributes;
                                    } else if ($scope.objectType == "REQUIREMENTDOCUMENTTYPE") {
                                        $scope.attributeValues = data.requirementObjectAttributes;
                                    } else if ($scope.objectType == "SUPPLIERTYPE") {
                                        $scope.attributeValues = data.supplierAttributes;
                                    }
                                    var map = new Hashtable();
                                    angular.forEach($scope.attributeValues, function (attribute) {
                                            ...
                                            if (value != null) {
                                                if (value.attachmentValues.length > 0) {
                                                    ...
                                                    AttributeAttachmentService.getMultipleAttributeAttachments(attachmentIds).then(
                                                        function (data) {
                                                            attribute.value.attachmentValues = data;
                                                        }, function (error) {
                                                            $rootScope.showErrorMessage(error.message);
                                                        }
                                                    )
                                                ...
                                                var decimalValue = 5;
                                                if (attribute.attributeDef.validations != null) {
                                                    var validations = JSON.parse(attribute.attributeDef.validations);
                                                    angular.forEach(validations, function (validation) {
                                                        if (validation.key == "NO_OF_DECIMALS_TO_DISPLAY" && validation.value != null && validation.value != "") {
                                                            decimalValue = validation.value;
                                                        }
                                                    })
                                                }
                                                ...
                                                if (attribute.attributeDef.dataType == 'LIST') {
                                                    ...
                                                    if (value.mlistValue == null && attribute.attributeDef.defaultListValue != null) {
                                                        attribute.value.mlistValue.push(attribute.attributeDef.defaultListValue);
                                                    }

                                                }
                                                if (attribute.attributeDef.dataType == 'MULTILISTCHECKBOX') {
                                                    if (value.mlistValue != null) {
                                                        attribute.value.mlistValue = value.mlistValue;
                                                    }
                                                }
                                                ...
                                                if (attribute.attributeDef.dataType == "LIST") {
                                                    ...
                                                    if (attribute.value.mlistValue != null && attribute.value.mlistValue.length == 0 && attribute.attributeDef.listMultiple == true
                                                        && attribute.attributeDef.defaultListValue != null) {
                                                        attribute.value.mlistValue.push(attribute.attributeDef.defaultListValue);
                                                    }
                                                }
                        ...
                        if (attribute.attributeDef.refType == 'ITEM') {
                            ...
                            WorkflowDefinitionService.getWorkflowAssignableTypeObjectType(value.refValue).then(
                                function (reqValue) {
                                    ...
                                    } else if (reqValue.objectType == "PPAP") {
                                        PpapService.getPpap(value.refValue).then(
                                            function (data) {
                                                attribute.value.refValue = data;
                                            }
                                        )
                                    } else if (reqValue.objectType == "PPAP") {
                                        SupplierAuditService.getSupplierAudit(value.refValue).then(
                                            function (data) {
                                                attribute.value.refValue = data;
                                            }
                                        )
                    ...
                    $scope.possibleTitle = parsed.html($translate.instant("POSSIBLE_VALUES")).html();
                    $scope.itemAttributePopover = {
                        templateUrl: 'app/desktop/modules/item/details/tabs/basic/itemAttributePopoverTemplate.jsp'
                    };
                    ...
                    $scope.checkToHideEditButton = checkToHideEditButton;
                    function checkToHideEditButton(attribute) {
                        var showEditButton = true;
                        if ($rootScope.toHideEditAttribute != undefined && $rootScope.toHideEditAttribute != null) {
                            if (attribute.attributeDef.id == $rootScope.toHideEditAttribute.id) {
                    ...
                    (function () {
                        ...
                        $scope.$on('app.attributes.tabActivated', function (event, data) {
                            if ($scope.objectType == "CUSTOMOBJECTTYPE") {
