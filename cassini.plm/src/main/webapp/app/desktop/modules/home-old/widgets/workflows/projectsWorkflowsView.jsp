<div class="responsive-table">
  <table class="table table-striped highlight-row">
    <thead>
    <tr style="font-size: 14px;">
      <th style="width: 100px" translate>Context</th>
      <th style="width: 100px" translate>NAME</th>
      <th style="width: 100px" translate>PERCENT_COMPLETE</th>
      <th style="width: 100px">Workflow Name</th>
      <th style="width: 100px" translate>CURRENT_ACTIVITY</th>
      <th style="width: 100px" translate>TIME_STAMP</th>
      <th style="width: 50px" translate>HISTORY
      </th>
    </tr>
    </thead>

    <tbody>
    <tr ng-if="workflowWigVm.loading == true">
      <td colspan="7">
                           <span style="font-size: 15px;">
                               <img src="app/assets/bower_components/cassini-platform/images/loaders/loader19.gif"
                                    class="mr5"><span translate>LOADING_ITEMS</span>
                           </span>
      </td>
    </tr>

    <tr ng-if="workflowWigVm.loading == false && workflowWigVm.workflows.content == 0">
      <td colspan="15" translate>MO_WORKFLOW</td>
    </tr>
    <tr ng-repeat="workflow in workflowWigVm.workflows.content" style="font-size: 14px;">
      <td style="width: 100px;">
        <span ng-if="workflow.project != null">PROJECT</span>

        <span ng-if="workflow.activity != null">ACTIVITY</span>

      </td>
      <td style="width: 100px;">
        <span ng-if="workflow.project != null"> <a href="" ng-click="workflowWigVm.showProjectDetails(workflow.project)">{{workflow.project.name}}</a></span>

        <span ng-if="workflow.activity != null"> <a href="" ng-click="workflowWigVm.showActivityDetails(workflow.activity)">{{workflow.activity.name}}</a></span>

      </td>
      <td style="width: 100px">
        <span ng-if="workflow.project != null">
          {{workflow.project.percentComplete}}%
        </span>
        <span ng-if="workflow.activity != null">
          {{workflow.activity.percentComplete}}%
        </span>
      </td>
      <td style="width: 100px">{{workflow.name}}</td>
      <td style="width: 100px">{{workflow.status}}</td>
      <td style="width: 100px">{{workflow.timeStamp}}</td>
      <td style="width: 50px; text-align: center" ng-click="workflowWigVm.showWorkflowHistory(workflow)"
          title="{{wfHistory}}">
        <i style="color: #337ab7;font-size: 18px;font-weight: 900" class="las la-sync"></i>
      </td>
    </tr>
    </tbody>
  </table>
</div>