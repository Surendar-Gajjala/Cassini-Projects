<div class="responsive-table">
    <table class="table table-striped highlight-row">
        ...
        <tr style="font-size: 14px;">
            ...
            <th style="width: 50px" translate>VERSION</th>
        ...
        <tr ng-repeat="workflow in workflowWigVm.workflows.content" style="font-size: 14px;">
            <td>
             <span ng-if="workflow.requirement != null">
                                <a href="" ng-click="workflowWigVm.showReqDetails(workflow.requirement)">{{workflow.requirement.objectNumber}}</a></span>
                <span ng-if="workflow.specification != null">
                                <a href="" ng-click="workflowWigVm.showSpecDetails(workflow.specification)">{{workflow.specification.objectNumber}}</a></span>
            </td>
            <td style="width: 100px">
                <span ng-if="workflow.requirement != null">{{workflow.requirement.name}}</span>
                <span ng-if="workflow.specification != null">{{workflow.specification.name}}</span>
            </td>
            <td style="width: 100px">
                <span ng-if="workflow.requirement != null">{{workflow.requirement.type.name}}</span>
                <span ng-if="workflow.specification != null">{{workflow.specification.type.name}}</span>
            </td>
            <td style="width: 50px">
                <span ng-if="workflow.requirement.version == 0">-</span>
                <span ng-if="workflow.requirement.version > 0">{{workflow.requirement.version}}</span>
            </td>
