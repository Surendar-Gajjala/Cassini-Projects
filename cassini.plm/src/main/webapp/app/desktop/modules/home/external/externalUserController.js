    ...
    function (module) {
        ...
        function ExternalUserController($scope, $rootScope, $sce, $translate, $cookieStore, $window, $timeout, $application, $state, $stateParams, $cookies, $uibModal,
            ...
            function showItem(item) {
                ...
                $window.localStorage.setItem("shared-permission", item.permission);
            ...
            function loadSharedItems() {
                ...
                if ($rootScope.externalUserShow == "Mfr") {
                    $rootScope.sharedMfr();
                } else if ($rootScope.externalUserShow == "MfrPart") {
                    $rootScope.sharedMfrPart();
                }
                else if ($rootScope.externalUserShow == "Supplier") {
                    $rootScope.sharedSupplier();
                }

                else if ($rootScope.externalUserShow == "CustomObject") {
                    $rootScope.sharedCustomObject();
                }

                else if ($rootScope.externalUserShow == "Folder") {
                    $rootScope.sharedFolder();
                }

                else if ($rootScope.externalUserShow == "Folders") {
                    $rootScope.sharedExternalUsersFoldersAndFiles();
                }

                else if ($rootScope.externalUserShow == "Declaration") {
                    $rootScope.showSharedDeclaration();
                } else if ($rootScope.externalUserShow == "Projects") {
            ...
            function loadFolders() {
                ...
                ShareService.getExternalSharedFoldersAndFiles(owner).then(
                    function (data) {
                        ...
                        rootNode = sharedFolderTree.tree('find', 0);
                        if (rootNode != null) {
                            rootNode.target.className = "tree-node tree-node-selected";
                        }
            ...
            function onSelectFolder(node) {
                if (node.attributes.folder != null && node.attributes.folder != undefined) {
                    $scope.objectId = node.attributes.folder.id;
                    $scope.type = node.attributes.folder.objectType;
                }
            ...
            function searchTree() {
                ...
                angular.forEach(vm.folderData, function (folder) {
                    if (folder.objectType == "FILE") {
                        ...
                        visitShareFolderChildren(folder, vm.searchValue);
                    } else {
                        folder.searchExist = false;
                        if (folder.name.toLowerCase().includes(vm.searchValue.toLowerCase())) {
                            folder.searchExist = true;
                        }
                        visitDocumentChildren(folder, vm.searchValue);
                    }
                });
                ...
                angular.forEach(vm.folderData, function (folder) {
                    if (folder.objectType == "FILE") {
                        if (folder.searchExist) {
                            var node = makeNode(folder);
                            if (folder.children != null && folder.children != undefined && folder.children.length > 0) {
                                node = visitShareFolderSearchChildren(node, folder.children);
                                node.state = "open";
                            }
                            nodes.push(node);
                        }
                        ...
                        if (folder.searchExist) {
                            var node = makeNode(folder);
                            if (folder.children != null && folder.children != undefined && folder.children.length > 0) {
                                node = visitDocumentSearchChildren(node, folder.children);
                                node.state = "open";
                            }
                            documentNodes.push(node);
                        }
            ...
            function visitDocumentSearchChildren(parent, folders) {
                var documentNodes = [];
                angular.forEach(folders, function (folder) {
                    if (folder.searchExist) {
                        var node = makeNode(folder);
                ...
                if (documentNodes.length > 0) {
                    parent.children = documentNodes;
                }
                return parent;
            }

            function visitDocumentChildren(parentNode, searchVal) {
                angular.forEach(parentNode.children, function (folder) {
                    folder.searchExist = false;
                    if (folder.name.toLowerCase().includes(searchVal.toLowerCase())) {
                        folder.searchExist = true;
                    }
                    folder = visitDocumentChildren(folder, searchVal);
                });
            ...
            function visitShareFolderChildren(parentNode, searchVal) {
                angular.forEach(parentNode.children, function (folder) {
                    folder.searchExist = false;
                    if (folder.name.toLowerCase().includes(searchVal.toLowerCase())) {
                        folder.searchExist = true;
                    }
                    folder = visitShareFolderChildren(folder, searchVal);
                });
            ...
            function visitShareFolderSearchChildren(parent, folders) {
                var nodes = [];
                angular.forEach(folders, function (folder) {
                    if (folder.searchExist) {
                        var node = makeNode(folder);
            ...
            function openActivityDetails(activity) {
                $window.localStorage.setItem("shared-permission", activity.permission);
                $state.go('app.pm.project.activity.details', {activityId: activity.id, tab: 'details.basic'})
            }
            ...
            function showTaskDetails(task) {
                ...
                $window.localStorage.setItem("shared-permission", task.permission);
            ...
            function showMfrDetails(mfr) {
                $rootScope.externalUserShow = null;
                $window.localStorage.setItem("shared-permission", mfr.permission);
                $state.go('app.mfr.details', {manufacturerId: mfr.id, tab: 'details.basic'});
                $rootScope.externalUserShow = "Mfr";
            }
            ...
            function showCustomObjectDetails(custom) {
                $rootScope.externalUserShow = null;
                $window.localStorage.setItem("shared-permission", custom.permission);
                $state.go('app.customobjects.details', {customId: custom.id, tab: 'details.basic'});
                $rootScope.externalUserShow = "CustomObject";
            }
            ...
            function showDeclaration(declaration) {
                $rootScope.externalUserShow = null;
                $window.localStorage.setItem("shared-permission", declaration.permission);
            ...
            function loadSharedCounts() {
                ShareService.getCounts("external").then(
                    function (data) {
            ...
            vm.downloadObjectFilesAsZip = downloadObjectFilesAsZip;
            function downloadObjectFilesAsZip() {
                $rootScope.showBusyIndicator($('.view-container'));
                var url = "{0}//{1}/api/plm/objects/{2}/{3}/files/zip".
                    format(window.location.protocol, window.location.host, $scope.objectId, "SHAREDOBJECT");
                window.open(url);
                $rootScope.hideBusyIndicator();
            }
            ...
            function filePreview(file) {
                var permission = $rootScope.hasPermission((file.objectType).toLowerCase(), "preview");
                if (permission) {
