...
describe('Items Module',function(){
    ...
    it('Check Item tab in the side navigation', function () {
        ...
        itemsButton.click();
        var assemblieitemsButton = element(by.id('assemblie-items'));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(assemblieitemsButton), 25000);
        assemblieitemsButton.click();
        browser.sleep(15000);
        expect(browser.getCurrentUrl()).toEqual(urlsPage.allAssembliesItems);
        browser.sleep(5000);
    });
    ...
    it('closed side pannel when we click close button(x)',function() {
        ...
        expect(newButton.getText()).toEqual('New Assembly');
        browser.sleep(8000);
    });
