...
//---------Click Machine Number hyperlink -> goto Basic details tab Successfully-------------
it('Click Machine Number hyperlink -> goto Basic details tab Successfully', function () {
    browser.get(urlsPage.allMachines);
    browser.sleep(8000);
    var number = element.all(by.css("[ng-click='allMachineVm.showMachine(machine)']")).first();
    ...
    browser.getCurrentUrl().then(function (url) {
        urlsPage.url_id = url.split("mes/machine/")[1].replace("?tab=details.basic","");
        browser.sleep(3000);
        expect(browser.getCurrentUrl())
            .toEqual(urlsPage.baseUrl + 'mes/machine/' +  urlsPage.url_id + '?tab=details.basic');
    });
...
describe('Check data update with Cancel option', function () {
    ...
    beforeAll(function () {
        browser.get(urlsPage.baseUrl + 'mes/machine/' +  urlsPage.url_id + '?tab=details.basic');
        browser.sleep(5000);
    });
    beforeEach(function () {
        browser.sleep(500);
    ...
    it('Name Field', function () {
        ...
        var beforeNameValue = element(by.css("[ng-bind-html='machineBasicVm.machine.name']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(beforeNameValue), 15000);

        var editClick = element(by.css("[ng-bind-html='machineBasicVm.machine.name']"));
        var EC = protractor.ExpectedConditions;
        ...
        browser.sleep(1500);
        var afterNameValue = element(by.css("[ng-bind-html='machineBasicVm.machine.name']"));
        var EC = protractor.ExpectedConditions;
    ...
    it('Description Field', function () {
        ...
        var beforeDescriptionValue = element(by.css("[editable-textarea='machineBasicVm.machine.description']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(beforeDescriptionValue), 15000);

        var editClick = element(by.css("[editable-textarea='machineBasicVm.machine.description']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(editClick), 15000);
        editClick.click();
        browser.sleep(1000);
        ...
        browser.sleep(1500);
        var afterDescriptionValue = element(by.css("[editable-textarea='machineBasicVm.machine.description']"));
        var EC = protractor.ExpectedConditions;
...
describe('Check data update with OK option', function () {
    ...
    it('Name Field', function () {
        ...
        browser.wait(EC.presenceOf(urlsPage.byModel('$parent.$data').clear().sendKeys('machine 27616516')), 35000);
        ...
        expect(plantUpdateMes).toEqual('Machine updated successfully');
        browser.sleep(5000);
