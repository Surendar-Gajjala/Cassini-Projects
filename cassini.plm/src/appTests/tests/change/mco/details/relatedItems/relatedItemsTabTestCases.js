...
describe('MCO details - Related Items view:', function () {
    beforeAll(function () {
        browser.refresh();
        browser.sleep(8000);
    });
    ...
    it('Click on Related Items tab -> open the  Related Items tab View successfully', function () {
        ...
        filesTab.click();
        expect(browser.getCurrentUrl()).toBe(urlsPage.baseUrl + 'changes/mco/' + urlsPage.url_id + '?tab=details.relatedItems');
        browser.sleep(5000);
    });
   
    it('Click Plus(+) button -> open Select Mfr Parts Side panel Successfully', function () {
        var plusButton = element(by.css('[ng-click="mcoRelatedItemsVm.addRelatedItems()"]'));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(plusButton), 15000);
        expect(plusButton.getAttribute('title')).toBe('Add Related Items');
        plusButton.click();
        browser.sleep(2000);
    ...
    it('Click Edit Item button -> Edit option enable successful', function () {
        ...
        browser.sleep(1500);
        var clickonEditItemButton = element(by.css("[spellcheck='false'] > .dropdown-menu [ng-click='mcoRelatedItemsVm.editItem(relatedItem)']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(clickonEditItemButton), 15000);
        clickonEditItemButton.click();
        browser.sleep(5000);
    
        var editNotesField = element(by.model("relatedItem.notes"));
        var updateButton = element(by.css('[ng-click="mcoRelatedItemsVm.updateItem(relatedItem)"]'));
        var cancelButton = element(by.css('[ng-click="mcoRelatedItemsVm.cancelChanges(relatedItem)"]'));
        browser.sleep(3000);
    ...
    it('Click Remove Item button -> remove that Mfr Part from the table Successful', function () {
        ...
        clickonActionButton.click();
        browser.sleep(1500);
        var clickonRemoveItemButton = element.all(by.css("[spellcheck='false'] > .dropdown-menu [ng-click='mcoRelatedItemsVm.deleteItem(relatedItem)']"));
        var EC = protractor.ExpectedConditions;
