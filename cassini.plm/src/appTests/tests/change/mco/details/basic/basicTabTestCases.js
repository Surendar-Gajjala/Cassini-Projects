...
it('Click Dco Number hyperlink -> goto Basic details tab Successfully', function () {
    ...
    browser.sleep(8000);
    var number = element.all(by.css("[ng-click='mcosVm.showMco(mco)']")).first();
    ...
    browser.getCurrentUrl().then(function (url) {
        urlsPage.url_id = url.split("mco/")[1].replace("?tab=details.basic","");
        browser.sleep(3000);
        expect(browser.getCurrentUrl())
            .toEqual(urlsPage.baseUrl+'changes/mco/'+urlsPage.url_id+'?tab=details.basic');
    });

    browser.sleep(5000);
});

xdescribe('MCO details -> Basic -> Check data update with Cancel option', function () {

    beforeAll(function () {
        browser.get(urlsPage.baseUrl + 'changes/mco/' +  urlsPage.url_id + '?tab=details.basic');
        browser.sleep(5000);
    });
    beforeEach(function () {
        browser.sleep(500);
    ...
    it('Title Field', function () {
        ...
        var beforeTitleValue = element(by.css("[editable-text='mcoBasicVm.mco.title']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(beforeTitleValue), 15000);

        var editClick = element(by.css("[editable-text='mcoBasicVm.mco.title']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(editClick), 15000);
        editClick.click();
        browser.sleep(1000);

        browser.wait(EC.presenceOf(urlsPage.byModel('$parent.$data').clear().sendKeys('Mco 275')), 35000);
        ...
        browser.sleep(1500);
        var afterTitleValue = element(by.css("[editable-text='mcoBasicVm.mco.title']"));
        var EC = protractor.ExpectedConditions;
    ...
    it('Description Field', function () {
        ...
        var beforeDescriptionValue = element(by.css("[editable-textarea='mcoBasicVm.mco.description']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(beforeDescriptionValue), 15000);

        var editClick = element(by.css("[editable-textarea='mcoBasicVm.mco.description']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(editClick), 15000);
        editClick.click();
        browser.sleep(1000);
        ...
        browser.sleep(1500);
        var afterDescriptionValue = element(by.css("[editable-textarea='mcoBasicVm.mco.description']"));
        var EC = protractor.ExpectedConditions;
    ...
    it('Reason for Change Field', function () {
        ...
        var beforeReasonForChangeValue = element(by.css("[editable-textarea='mcoBasicVm.mco.reasonForChange']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(beforeReasonForChangeValue), 15000);

        var editClick = element(by.css("[editable-textarea='mcoBasicVm.mco.reasonForChange']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(editClick), 15000);
        editClick.click();
        browser.sleep(1000);
        ...
        browser.sleep(1500);
        var afterReasonForChangeValue = element(by.css("[editable-textarea='mcoBasicVm.mco.reasonForChange']"));
        var EC = protractor.ExpectedConditions;
...
xdescribe('MCO details -> Basic -> Check data update with OK option', function () {
    ...
    it('Title Field', function () {
        ...
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(alertMessage), 5000, "Text is not something I've expected");
        expect(alertMessage).toEqual('MCO updated successfully');
        browser.sleep(5000);
    ...
    it('Change Analyst Field', function () {
        ...
        var editClick = element(by.css("[editable-select='mcoBasicVm.mco.changeAnalystObject']"));
        var EC = protractor.ExpectedConditions;
        browser.wait(EC.presenceOf(editClick), 15000);
        editClick.click();
        browser.sleep(1000);
