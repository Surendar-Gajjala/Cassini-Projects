define(
    [
        'app/desktop/desktop.app',
        'app/shared/services/testTreeService'
    ],
    function (module) {
        module.controller('TestTreeController', TestTreeController);

        function TestTreeController($scope, $rootScope, $timeout, $state, $stateParams, $cookies, TestTreeService) {
            if ($application.homeLoaded == true) {
                return;
            }

            var vm = this;

            var nodeId = 0;
            var testTree = null;
            var rootNode = null;


            function onSelectType(node) {
                var data = testTree.tree('getData', node.target);
                var type = data.attributes.type;
                window.$("body").trigger("click");
            }

            function initClassificationTree() {
                testTree = $('#testTree').tree({
                    data: [
                        {
                            id: nodeId,
                            text: 'Test',
                            iconCls: 'classification-root',
                            object: null,
                            children: [],
                            onSelect: onSelectType
                        }
                    ]
                });

                rootNode = testTree.tree('find', 0);
            }

            function loadClassificationTree() {
                TestTreeService.getTestTree().then(
                    function (data) {
                        var nodes = [];
                        angular.forEach(data, function (type) {
                            var node = makeNode(type);

                            if (type.children != null && type.children != undefined && type.children.length > 0) {
                                node.state = "closed";
                                visitChildren(node, type.children);
                            }

                            nodes.push(node);
                        });

                        testTree.tree('append', {
                            parent: rootNode.target,
                            data: nodes
                        });
                    }
                )
            }

            function visitChildren(parent, itemTypes) {
                var nodes = [];
                angular.forEach(itemTypes, function (type) {
                    var node = makeNode(type);

                    if (type.children != null && type.children != undefined && type.children.length > 0) {
                        node.state = 'closed';
                        visitChildren(node, type.children);
                    }

                    nodes.push(node);
                });

                if (nodes.length > 0) {
                    parent.children = nodes;
                }
            }

            function makeNode(type) {
                return {
                    id: ++nodeId,
                    text: type.name,
                    iconCls: '',
                    object: type
                };
            }

            (function () {
                $scope.$on('$viewContentLoaded', function () {
                    initClassificationTree();
                    loadClassificationTree();
                });
            })();
        }
    }
);