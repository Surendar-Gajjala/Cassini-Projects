...
<div class="styled-panel permissions-table" ng-show="pusVm.showGrid">
  <table>
    ...
    <tr>
      <th class="skew">
        <div class="" style="width: auto;"><span>Assistors</span></div>
      </th>
    ...
    <tr ng-repeat="assistor in pusVm.assistors">
      <td nowrap class="permission-name"
          ng-init="pusVm.first = false" ng-class="{'last': (pusVm.last && $last)}">
        <span style="">{{assistor.fullName}}</span>
      </td>
      <td align="center"
          ng-class="{'odd': $index %2 != 1,
                                   'first': pusVm.first, 'last':  (pusVm.last && $parent.$last)}"
        ...
        <input type="checkbox"
               style="text-align:center"
               ng-init="permIndex = pusVm.getResponderIndexInGroup(assistor, utility);"
               ng-model="assistor.utilities[permIndex].selected"
               ng-click="pusVm.toggleResponderUtilities(assistor, assistor.utilities[permIndex])"/>
      </td>
    </tr>
    </tbody>
  </table>
