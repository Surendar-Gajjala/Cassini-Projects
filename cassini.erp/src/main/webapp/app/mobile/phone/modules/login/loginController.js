            ...
            [
                ...
                function ($scope, $rootScope, $timeout, $interval, $state, $stateParams, $cookies,
                    ...
                    function setBgImage(index) {
                        ...
                        $('html').css('background-image', 'url(app/assets/images/bg/' + image + ')');
                    ...
                    $scope.onSwipeRight = function() {
                        ...
                        $cookies.put('_BG_INDEX_', bgIndex);
                        $('html').css('background-image', 'url(app/assets/images/bg/' + bgImages[bgIndex] + ')');
                    };
                    ...
                    $scope.bringFormToView = function() {
                    ...
                    $scope.performLogin = function(event) {
                        loginFactory.login($scope.login.username, $scope.login.password).then(
                            function(session) {
                                $app.session = session;
                                $app.login = session.login;

                                $cookies.put('_CASSINI_USERNAME_', $scope.login.username);
                                $cookies.put('_CASSINI_PASSWORD_', $scope.login.password);
                                $cookies.put('_CASSINI_REMEMBERME_', $scope.login.username);

                                if(App2AndroidBridge.isPhoneAvailable()) {
                                    App2AndroidBridge.setAuthentication($scope.login.username, $scope.login.password);
                                    setDeviceInfo(event);
                                }
                                else {
                            ...
                            function (error) {
                                ...
                                $scope.showAlert(event, error.message);
                            }
                        )
                    };
                    ...
                    function setDeviceInfo(event) {
                        App2AndroidBridge.getDeviceInfo(function(json) {
                            ...
                            loginFactory.setMobileDevice($app.session.id, device).then(
                                function(data) {
                                    //$scope.showAlert(event, JSON.stringify(data));
                                    $state.go('app.home', {}, { reload: true });
                                }
                            );
                        });
