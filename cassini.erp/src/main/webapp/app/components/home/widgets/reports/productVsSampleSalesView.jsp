<div class="panel panel-default panel-alt widget-messaging" style="height: 500px">
    <div class="panel-heading" style="background-color: #E4E7EA; padding:0; height: 56px">
        <div class="row" style="padding-right: 10px;">
            <div class="panel-title col-sm-4" style="font-size:15px; padding: 20px 0 0 20px">Product vs Sample Sales</div>
        ...
        <treasure-overlay-spinner active='spinner.active == true'>
            <div class='responsive-table' style="min-height: 440px; max-height: 440px; overflow: auto;">
                <table class='table table-striped'>
                    <thead>
                    <tr>
                        <th style="cursor: pointer" ng-click="order('customer')">
                            Customer
                            <i ng-if="predicate == 'customer'" class="fa" ng-class="{'fa-sort-desc': reverse, 'fa-sort-asc': !reverse}"></i>
                        </th>
                    ...
                    <tbody>
                    <tr ng-repeat="row in orders  | filter:filterText">
                        <td><a href="" ng-click="showCustomerDetails(row.customerId)">{{row.customer}}</a></td>
                        <td>{{row.product | currency:"&#x20b9; ":0}}</td>
                        <td>{{row.sample | currency:"&#x20b9; ":0}}</td>
                    ...
                    <tr ng-if='spinner.active == false'>
                        ...
                        <td>
                            <strong>{{totals.product | currency:"&#x20b9; ":0}}</strong>
                        </td>
                        <td>
                            <strong>{{totals.sample | currency:"&#x20b9; ":0}}</strong>
                        </td>
                        <td class="text-center">
                            <strong><span class="label" ng-class="{'label-danger': totals.percent > 10, 'label-success':totals.percent<=10}">{{totals.percent}}%</span></strong>
                        </td>
                    </tr>
                    </tbody>
                </table>
