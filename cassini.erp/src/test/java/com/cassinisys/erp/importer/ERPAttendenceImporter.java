...
@Component
public class ERPAttendenceImporter {
	...
	/*public static void main(String[] args) {
		try {
			Testt xlReader = new Testt();
			// getDate("03-Nov-2015", "09:39");
			xlReader.init("D://Cassini Lst/Daily Attendance Report 02 nov 2016.xls");
...
*/	@Test
	@Rollback(false)
	public void importAttendenceData(){

		String path="E://Cassini Sys/BasicWorkDurationReport exl.xls";
		List<ERPEmployeeAttendance> attendances= init(path);

		for(ERPEmployeeAttendance attendance:attendances){
...
//if no employees in DB uncomment below code
	...
	for(int i=0;i<150;i++){
		...
		int j=i+1;
		emp.setEmployeeNumber(j+"");
		emp.setFirstName("koti"+emp.getEmployeeNumber());
		emp.setLastName(" reddy"+emp.getEmployeeNumber());
...
*/
	...
	public List<ERPEmployeeAttendance> init(String filePath) {
		...
		try {
			...
			if (attnds.size() > 0) {
				...
				for (ERPEmployeeAttendance attDTO : attnds) {

					System.out.println(" Emp Code:" + attDTO.getEmpNumber());

					System.out.println(" Date and  In Time : "
							+ attDTO.getInTime());
					System.out.println(" Date and  Out Time : "
							+ attDTO.getOutTime());
